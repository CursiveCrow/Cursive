...
def find_compiler(root: Path) -> Path:
    """Locate cursivec0.exe relative to spec-tests directory."""
    candidates = [
...
def discover_tests(root: Path, filter_pattern: Optional[str] = None) -> List[TestCase]:
    """Discover all test cases under the spec-tests directory."""
    tests: List[TestCase] = []
...
def parse_compiler_output(output: str) -> List[Diagnostic]:
    ...
    # Or: severity[code]: message (no location)
    pattern = re.compile(
        r'(?:([^:]+):(\d+):(\d+):\s*)?'  # optional location
        r'(error|warning|info)\[([A-Z]-[A-Z]{3}-\d{4})\]:\s*(.+)',
        re.IGNORECASE
    )
...
'''
        ...
        compile_result = subprocess.run(
            [str(config.compiler_path), str(tmp_path)],
            capture_output=True,
            text=True,
        ...
        if directives.run_mode == RunMode.COMPILE_FAIL:
            ...
            for ld in directives.line_directives:
                matching = [d for d in diagnostics if d.code == ld.code and d.line == ld.line]
                if not matching:
                    return TestOutcome(
                        test=test,
                        result=TestResult.FAIL,
                        duration=time.time() - start_time,
                        message=f"Line {ld.line}: Expected {ld.severity} {ld.code}, not found",
                        diagnostics=diagnostics,
                        stdout=compile_result.stdout,
                        stderr=compile_result.stderr,
                        exit_code=compile_result.returncode,
                ...
                if ld.message:
                    ...
                    if not msg_match:
                        return TestOutcome(
                            ...
                            result=TestResult.FAIL,
                            duration=time.time() - start_time,
                            message=f"Line {ld.line}: {ld.code} message mismatch, expected substring '{ld.message}'",
                            diagnostics=diagnostics,
                            stdout=compile_result.stdout,
                            stderr=compile_result.stderr,
                            exit_code=compile_result.returncode,
...
def run_project_test(test: TestCase, config: Config, start_time: float) -> TestOutcome:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Run compiler
        compile_result = subprocess.run(
            [str(config.compiler_path), str(project_path)],
            capture_output=True,
            text=True,
            timeout=config.timeout,
...
def report_junit(outcomes: List[TestOutcome], config: Config) -> None:
    ...
    suite = ET.SubElement(root, "testsuite", name="cursive0-spec-tests")
