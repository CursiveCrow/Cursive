...
using cursive0::analysis::ResolveContext;
using cursive0::analysis::ResolveItem;
using cursive0::analysis::ResolveModule;
using cursive0::analysis::ResolveModules;
...
int main() {
  {
    ...
    const auto local_res = cursive0::analysis::ResolveType(
        env.res_ctx, MakeTypePathType("Local"));
    assert(local_res.ok);

    const auto alias_res = cursive0::analysis::ResolveType(
        env.res_ctx, MakeTypePathType("Alias"));
    assert(alias_res.ok);

    const auto qual_res = cursive0::analysis::ResolveType(
        env.res_ctx, MakeTypePathType(TypePath{"pkg", "QualType"}));
    assert(qual_res.ok);

    const auto dyn_res = cursive0::analysis::ResolveType(
        env.res_ctx, MakeTypeDynamic(ClassPath{"Cls"}));
    assert(dyn_res.ok);

    const auto dyn_qual_res = cursive0::analysis::ResolveType(
        env.res_ctx, MakeTypeDynamic(ClassPath{"pkg", "QualClass"}));
    assert(dyn_qual_res.ok);

    const auto modal_res = cursive0::analysis::ResolveType(
        env.res_ctx, MakeTypeModalState("Local", "State"));
    assert(modal_res.ok);

    const auto hom_res = cursive0::analysis::ResolveType(
