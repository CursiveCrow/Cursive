...
using cursive0::eval::BindState;
using cursive0::eval::BindVal;
using cursive0::eval::Binding;
using cursive0::eval::Cleanup;
using cursive0::eval::CleanupItem;
using cursive0::eval::CleanupScope;
using cursive0::eval::CleanupStatus;
using cursive0::eval::ControlKind;
using cursive0::eval::DropActionOut;
using cursive0::eval::DropStaticActionOut;
using cursive0::eval::DropStatus;
using cursive0::eval::DropSubvalue;
using cursive0::eval::DropValueOut;
using cursive0::eval::Movability;
using cursive0::eval::Responsibility;
using cursive0::eval::SetState;
using cursive0::eval::Sigma;
using cursive0::eval::SemanticsContext;
using cursive0::eval::UnitVal;
...
void TestCleanupListRules() {
...

  {
    ...
    const auto status = Cleanup(ctx, {cursive0::eval::DropBinding{binding}}, sigma);
    assert(status == CleanupStatus::Ok);
  }
  ...
  {
    Sigma sigma;
    const auto path = cursive0::analysis::PathKeyOf(cursive0::syntax::ModulePath{"main"});
    ...
    const auto status = Cleanup(
        ctx, {cursive0::eval::DropStatic{path, "x"}}, sigma);
    assert(status == CleanupStatus::Ok);
  }

  {
    ...
    const auto path = cursive0::analysis::PathKeyOf(cursive0::syntax::ModulePath{"main"});
    const auto status = Cleanup(
        ctx, {cursive0::eval::DropStatic{path, "missing"}}, sigma);
    assert(status == CleanupStatus::Panic);
  }

  {
    ...
    const auto status = Cleanup(
        ctx, {cursive0::eval::DeferBlock{block}}, sigma);
    assert(status == CleanupStatus::Ok);
  }
...
void TestCleanupScopeSteps() {
...

  {
    ...
    Binding binding;
    BindVal(sigma, "x", Value{UnitVal{}}, BindInfo{}, &binding);
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DropBinding{binding}};
...

  {
    ...
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DropBinding{bad}};
    const auto status = CleanupScope(ctx, *scope, sigma);
    assert(status == CleanupStatus::Panic);
  }

  {
    ...
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DropBinding{bad2},
                      cursive0::eval::DropBinding{bad1}};
    const auto status = CleanupScope(ctx, *scope, sigma);
...

  {
    ...
    cursive0::eval::PushScope(sigma, nullptr);
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DropStatic{path, "x"}};
    const auto status = CleanupScope(ctx, *scope, sigma);
    assert(status == CleanupStatus::Ok);
  }

  {
    ...
    cursive0::eval::PushScope(sigma, nullptr);
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DropStatic{path, "missing"}};
    const auto status = CleanupScope(ctx, *scope, sigma);
    assert(status == CleanupStatus::Panic);
  }

  {
    ...
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DropStatic{path, "missing"},
                      cursive0::eval::DropStatic{path, "missing"}};
    const auto status = CleanupScope(ctx, *scope, sigma);
    assert(status == CleanupStatus::Abort);
  }

  {
    ...
    cursive0::eval::PushScope(sigma, nullptr);
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DeferBlock{MakeEmptyBlock()}};
    const auto status = CleanupScope(ctx, *scope, sigma);
    assert(status == CleanupStatus::Ok);
  }

  {
    ...
    cursive0::eval::PushScope(sigma, nullptr);
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DeferBlock{MakeErrorBlock()}};
    const auto status = CleanupScope(ctx, *scope, sigma);
    assert(status == CleanupStatus::Panic);
  }

  {
    ...
    auto* scope = cursive0::eval::CurrentScope(sigma);
    scope->cleanup = {cursive0::eval::DeferBlock{MakeErrorBlock()},
                      cursive0::eval::DeferBlock{MakeErrorBlock()}};
    const auto status = CleanupScope(ctx, *scope, sigma);
...
void TestDropActionAndDestroy() {
...

  {
    ...
    sema_ctx.sigma.types[cursive0::analysis::PathKeyOf({"Rec"})] = rec;
    ...
    Sigma sigma;
    cursive0::eval::PushScope(sigma, nullptr);
    cursive0::eval::RecordVal rec_val;
    rec_val.record_type = cursive0::analysis::MakeTypePath({"Rec"});
...

  {
    ...
    Binding binding;
    BindVal(sigma, "x", Value{UnitVal{}}, BindInfo{}, &binding);
    cursive0::eval::DestroyBindings(ctx, {binding}, sigma);
    cursive0::eval::DestroyBindings(ctx, {}, sigma);
  }
}
...
void TestDropSubvalueAndDropValueOut() {
...

  {
    ...
    RecordDecl needs_drop = MakeRecordDecl(
        ...
        {},
        {ClassPath{"Drop"}});
    sema_ctx.sigma.types[cursive0::analysis::PathKeyOf({"NeedsDrop"})] = needs_drop;
    ...
    Sigma sigma;
    cursive0::eval::RecordVal rec_val;
    rec_val.record_type = cursive0::analysis::MakeTypePath({"NeedsDrop"});
    rec_val.fields = {};
    const auto status = DropValueOut(ctx_drop,
                                     cursive0::analysis::MakeTypePath({"NeedsDrop"}),
                                     Value{rec_val},
                                     {},
                                     sigma);
...

  {
    ...
    sema_ctx.sigma.types[cursive0::analysis::PathKeyOf({"NeedsDrop"})] = needs_drop;
    sema_ctx.sigma.types[cursive0::analysis::PathKeyOf({"Parent"})] = parent;

    SemanticsContext ctx_drop;
    ctx_drop.sema = &sema_ctx;

    cursive0::eval::RecordVal child_val;
    child_val.record_type = cursive0::analysis::MakeTypePath({"NeedsDrop"});
    child_val.fields = {};

    cursive0::eval::RecordVal parent_val;
    parent_val.record_type = cursive0::analysis::MakeTypePath({"Parent"});
    parent_val.fields = {{"child", Value{child_val}}};

    Sigma sigma;
    const auto status = DropValueOut(ctx_drop,
                                     cursive0::analysis::MakeTypePath({"Parent"}),
                                     Value{parent_val},
                                     {},
                                     sigma);
    assert(status == DropStatus::Panic);
