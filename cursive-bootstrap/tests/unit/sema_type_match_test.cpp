...
using cursive0::sema::CheckMatchExpr;
...
using cursive0::sema::TypeMatchExpr;
using cursive0::sema::TypeMatchPattern;
...
int main() {
  ...
  // Match typing: enum
  {
    ...
    assert(typed.ok);
    assert(typed.type);
    const auto* prim = std::get_if<cursive0::sema::TypePrim>(&typed.type->node);
