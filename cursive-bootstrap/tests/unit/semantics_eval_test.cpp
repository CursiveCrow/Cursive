...
using cursive0::semantics::BreakVal;
using cursive0::semantics::EvalFieldInitsSigma;
using cursive0::semantics::EvalListSigma;
using cursive0::semantics::EvalOptSigma;
using cursive0::semantics::EvalSigma;
...
using cursive0::semantics::ValueEqual;
...
static void AddName(cursive0::sema::NameMapTable& maps,
  ...
  map.emplace(
      cursive0::sema::IdKeyOf(name),
      cursive0::sema::Entity{kind, std::move(origin), std::nullopt,
...
static Value MakeBool(bool value) {
  return Value{cursive0::semantics::BoolVal{value}};
}
...
int main() {
  ...
  SetExprType(expr_types, lit_i32, cursive0::sema::MakeTypePrim("i32"));
  ...
  assert(std::holds_alternative<cursive0::semantics::Control>(opt_ctrl));
  ...
  SetExprType(expr_types, lit_i32_b, cursive0::sema::MakeTypePrim("i32"));
  ...
  SetExprType(expr_types, lit_usize_lo, cursive0::sema::MakeTypePrim("usize"));
  SetExprType(expr_types, lit_usize_hi, cursive0::sema::MakeTypePrim("usize"));
  ...
  assert(std::holds_alternative<cursive0::semantics::RangeVal>(std::get<Value>(range_out.node).node));
  ...
  const auto* ptr_val = std::get_if<cursive0::semantics::PtrVal>(&std::get<Value>(ptr_out.node).node);
  assert(ptr_val && ptr_val->state == cursive0::sema::PtrState::Null && ptr_val->addr == 0);
  ...
  SetExprType(expr_types, record_expr, cursive0::sema::MakeTypePath({"Point"}));
  ...
  SetExprType(expr_types, idx_oob_lit, cursive0::sema::MakeTypePrim("usize"));
  ...
  assert(std::holds_alternative<cursive0::semantics::SliceVal>(std::get<Value>(idx_range_out.node).node));

  auto lit_usize_oob = MakeLiteralExpr(cursive0::syntax::TokenKind::IntLiteral, "10");
  SetExprType(expr_types, lit_usize_oob, cursive0::sema::MakeTypePrim("usize"));
...

  {
    ...
    const auto mod_key = cursive0::sema::PathKeyOf(mod_path);
    const auto poison_addr = cursive0::semantics::AllocAddr(sigma_poison);
    sigma_poison.store[poison_addr] = MakeBool(true);
    sigma_poison.poison_flags[mod_key] = poison_addr;

    cursive0::sema::ScopeContext sema_poison;
    cursive0::sema::Entity ent;
    ent.kind = cursive0::sema::EntityKind::Value;
    ent.origin_opt = mod_path;
    ent.source = cursive0::sema::EntitySource::Decl;
    cursive0::sema::Scope scope;
    scope.emplace(cursive0::sema::IdKeyOf("pval"), ent);
...

  {
    ...
    cursive0::sema::ScopeContext sema_poison;
    cursive0::sema::Entity ent;
    ent.kind = cursive0::sema::EntityKind::Type;
    ent.origin_opt = mod_path;
    ent.source = cursive0::sema::EntitySource::Decl;
    cursive0::sema::Scope scope;
    scope.emplace(cursive0::sema::IdKeyOf("Rec"), ent);
    sema_poison.scopes = {scope};
...

  {
    ...
    const auto mod_key = cursive0::sema::PathKeyOf(mod_path);
    const auto addr = cursive0::semantics::AllocAddr(sigma_path);
    sigma_path.store[addr] = MakeInt("i32", 99);
    sigma_path.static_addrs[{mod_key, "pval"}] = addr;

    cursive0::sema::ScopeContext sema_path;
    sema_path.current_module = current;

    cursive0::sema::NameMapTable name_maps;
    name_maps.emplace(cursive0::sema::PathKeyOf(current),
                      cursive0::sema::NameMap{});
    name_maps.emplace(mod_key, cursive0::sema::NameMap{});
    AddName(name_maps, mod_path, "pval", cursive0::sema::EntityKind::Value,
            mod_path);

    cursive0::sema::ModuleNames module_names = {
...

  {
    ...
    sema_path.sigma.types.emplace(cursive0::sema::PathKeyOf(type_path),
                                  MakeRecordDecl("Rec"));
    ...
    name_maps.emplace(cursive0::sema::PathKeyOf(current),
                      cursive0::sema::NameMap{});
    name_maps.emplace(mod_key, cursive0::sema::NameMap{});
    AddName(name_maps, mod_path, "Rec", cursive0::sema::EntityKind::Type,
            mod_path);
  ...
  SetExprType(expr_types, bool_true, cursive0::sema::MakeTypePrim("bool"));
  SetExprType(expr_types, bool_false, cursive0::sema::MakeTypePrim("bool"));
...

  {
    ...
    cursive0::semantics::IntVal expected;
    expected.type = "i32";
...

  {
    ...
    SetExprType(expr_types, lit_f32, cursive0::sema::MakeTypePrim("f32"));
...

  {
    ...
    SetExprType(expr_types, lit_zero, cursive0::sema::MakeTypePrim("i32"));
...

  {
    ...
    SetExprType(expr_types, cast_expr, cursive0::sema::MakeTypePrim("i32"));
...

  {
    ...
    auto cast_expr = MakeExpr(std::move(cast));
    SetExprType(expr_types, cast_expr, cursive0::sema::MakeTypePrim("char"));
    Outcome cast_out = EvalSigma(ctx, *cast_expr, sigma);
...

  {
    ...
    SetExprType(expr_types, trans_expr, cursive0::sema::MakeTypePrim("u32"));
  ...
  {
    auto type_i32 = cursive0::sema::MakeTypePrim("i32");
    auto type_bool = cursive0::sema::MakeTypePrim("bool");
    auto union_type = cursive0::sema::MakeTypeUnion({type_i32, type_bool});
    ctx.ret_type = type_i32;

    Value union_ok;
    cursive0::semantics::UnionVal uni_ok;
    ...
    Value union_err;
    cursive0::semantics::UnionVal uni_err;
    ...
    const auto ctrl = std::get<cursive0::semantics::Control>(prop_err_out.node);
    assert(ctrl.kind == cursive0::semantics::ControlKind::Return);
...

  {
    ...
    const auto* ptr =
        std::get_if<cursive0::semantics::PtrVal>(&std::get<Value>(addr_out.node).node);
    assert(ptr && ptr->state == cursive0::sema::PtrState::Valid);
...

  {
    ...
    cursive0::semantics::RawPtrVal raw_ptr;
    raw_ptr.qual = cursive0::sema::RawPtrQual::Imm;
  ...
  {
    cursive0::semantics::ArrayVal arr;
    ...
    SetExprType(expr_types, idx_oob, cursive0::sema::MakeTypePrim("usize"));
    auto addr_oob_expr =
...

  {
    ...
    const auto binding = cursive0::semantics::LookupBind(sigma, "x");
    assert(binding.has_value());
    const auto state = cursive0::semantics::BindStateOf(sigma, *binding);
    assert(state.has_value());
    assert(state->kind == cursive0::semantics::BindState::Kind::Moved);
  }

  {
    cursive0::semantics::RecordVal rec;
    rec.record_type = cursive0::sema::MakeTypePath({"Point"});
    rec.fields = {{"x", MakeInt("i32", 8)}, {"y", MakeInt("i32", 9)}};
    cursive0::semantics::Binding rec_binding;
    ...
    Outcome move_out = EvalSigma(ctx, *move_expr, sigma);
    assert(IsVal(move_out));
    const auto state = cursive0::semantics::BindStateOf(sigma, rec_binding);
    assert(state.has_value());
    assert(state->kind == cursive0::semantics::BindState::Kind::PartiallyMoved);
    assert(state->fields.count("x") == 1);
  }

  {
    cursive0::semantics::RegionEntry region;
    region.tag = 1;
    region.target = 1;
    region.scope = cursive0::semantics::CurrentScopeId(sigma);
    ...
    cursive0::semantics::RecordVal region_val;
    region_val.record_type = cursive0::sema::MakeTypeModalState({"Region"}, "@Active");
