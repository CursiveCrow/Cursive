...
int main() {
  {
    ...
    const auto perms = cursive0::analysis::CheckC0SubsetPermSyntax(source);
...

  {
    ...
    const auto source = LoadSourceChecked(content);
    const auto uns = cursive0::analysis::CheckC0UnsupportedConstructs(source);
...

  {
    ...
    ast.module_doc = {};
    ast.unsafe_spans = {};
    const auto uns = cursive0::analysis::CheckC0UnsupportedFormsAst(ast, source);
    assert(!uns.subset_ok);
    assert(!uns.diags.empty());
  }
