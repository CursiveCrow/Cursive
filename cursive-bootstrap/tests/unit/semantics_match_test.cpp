...
using cursive0::semantics::BindEnv;
using cursive0::semantics::MatchPattern;
using cursive0::semantics::MatchRecord;
...
static Value MakeTuple(std::vector<Value> elements) {
  cursive0::semantics::TupleVal v;
  v.elements = std::move(elements);
  return Value{v};
}

static Value MakeRecord(std::vector<std::pair<std::string, Value>> fields) {
  cursive0::semantics::RecordVal v;
  v.record_type = cursive0::sema::MakeTypePath({"Point"});
  v.fields = std::move(fields);
  return Value{v};
}

static Value MakeEnumUnit(cursive0::sema::TypePath path) {
  cursive0::semantics::EnumVal v;
  v.path = std::move(path);
  return Value{v};
}

static Value MakeEnumTuple(cursive0::sema::TypePath path,
                           std::vector<Value> elements) {
  cursive0::semantics::EnumVal v;
  v.path = std::move(path);
  cursive0::semantics::EnumPayloadTupleVal payload;
  payload.elements = std::move(elements);
  v.payload = std::move(payload);
  return Value{v};
}

static Value MakeEnumRecord(cursive0::sema::TypePath path,
                            std::vector<std::pair<std::string, Value>> fields) {
  cursive0::semantics::EnumVal v;
  v.path = std::move(path);
  cursive0::semantics::EnumPayloadRecordVal payload;
  payload.fields = std::move(fields);
  v.payload = std::move(payload);
  return Value{v};
}
...
static Value MakeModal(std::string state, Value payload) {
  cursive0::semantics::ModalVal v;
  v.state = std::move(state);
  v.payload = std::make_shared<Value>(std::move(payload));
  return Value{v};
}

static Value MakeUnionValue(cursive0::sema::TypeRef member, Value payload) {
  cursive0::semantics::UnionVal v;
...
int main() {
  ...
  Value bool_value{cursive0::semantics::BoolVal{true}};
  ...
  Value union_value = MakeUnionValue(cursive0::sema::MakeTypePrim("i32"), int_one);
