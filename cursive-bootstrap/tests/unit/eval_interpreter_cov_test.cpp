...
void TestInterpretProjectInitPanic() {
  ...
  cursive0::analysis::ExprTypeMap expr_types;
  expr_types[main_expr.get()] = cursive0::analysis::MakeTypePrim("i32");
  ScopeContext scope = MakeScope(mod, &expr_types);

  cursive0::eval::Sigma sigma;
  const auto project = MakeProject();
  auto result = cursive0::eval::InterpretProject(project, sigma, scope);
  assert(result.has_control);
  assert(result.control.kind == cursive0::eval::ControlKind::Panic);
}
...
void TestInterpretProjectDeinitPanic() {
  ...
  cursive0::analysis::ExprTypeMap expr_types;
  expr_types[main_expr.get()] = cursive0::analysis::MakeTypePrim("i32");
  expr_types[bad_static.binding.init.get()] = cursive0::analysis::MakeTypePrim("i32");
  ScopeContext scope = MakeScope(mod, &expr_types);
