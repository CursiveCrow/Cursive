...
static cursive0::analysis::ModuleNames ModuleNamesFromScope(
    const cursive0::analysis::ScopeContext& ctx) {
  cursive0::analysis::ModuleNames names;
...
void TestEvalArgsAndCallRules() {
  ...
  cursive0::analysis::ScopeContext scope;
  ...
  SetExprType(expr_types, rec_init, cursive0::analysis::MakeTypePrim("i32"));

  auto lit_arg = MakeLiteralExpr(TokenKind::IntLiteral, "2");
  SetExprType(expr_types, lit_arg, cursive0::analysis::MakeTypePrim("i32"));
  ...
  BindVal(sigma, "x", cursive0::eval::Value{cursive0::eval::UnitVal{}},
          BindInfo{}, &binding);
  sigma.store[binding.addr] = cursive0::eval::Value{cursive0::eval::UnitVal{}};

  auto call_empty = MakeCallExpr(MakeIdentifierExpr("proc_empty"), {});
  auto out_empty = cursive0::eval::EvalSigma(ctx, *call_empty, sigma);
  assert(IsVal(out_empty));

  auto call_move_ok =
      MakeCallExpr(MakeIdentifierExpr("proc_move"), {MakeArg(lit_arg)});
  auto out_move_ok = cursive0::eval::EvalSigma(ctx, *call_move_ok, sigma);
  assert(IsVal(out_move_ok));

  auto call_ref_ok =
      MakeCallExpr(MakeIdentifierExpr("proc_ref"),
                   {MakeArg(MakeIdentifierExpr("x"))});
  auto out_ref_ok = cursive0::eval::EvalSigma(ctx, *call_ref_ok, sigma);
  assert(IsVal(out_ref_ok));

  auto call_move_ctrl =
      MakeCallExpr(MakeIdentifierExpr("proc_move"), {MakeArg(MakeErrorExpr())});
  auto out_move_ctrl = cursive0::eval::EvalSigma(ctx, *call_move_ctrl, sigma);
  assert(IsCtrl(out_move_ctrl));

  auto call_ref_ctrl =
      MakeCallExpr(MakeIdentifierExpr("proc_ref"),
                   {MakeArg(MakeIdentifierExpr("missing"))});
  auto out_ref_ctrl = cursive0::eval::EvalSigma(ctx, *call_ref_ctrl, sigma);
  assert(IsCtrl(out_ref_ctrl));

  auto call_callee_ctrl = MakeCallExpr(MakeErrorExpr(), {});
  auto out_callee_ctrl = cursive0::eval::EvalSigma(ctx, *call_callee_ctrl, sigma);
  assert(IsCtrl(out_callee_ctrl));

  auto call_record = MakeCallExpr(MakeIdentifierExpr("Rec"), {});
  auto out_record = cursive0::eval::EvalSigma(ctx, *call_record, sigma);
  assert(IsVal(out_record));

  auto call_region =
      MakeCallExpr(MakeExpr(PathExpr{{"Region"}, "new_scoped"}),
                   {MakeArg(MakeIdentifierExpr("x"))});
  auto out_region = cursive0::eval::EvalSigma(ctx, *call_region, sigma);
  assert(IsVal(out_region));

  auto call_region_ctrl =
      MakeCallExpr(MakeExpr(PathExpr{{"Region"}, "new_scoped"}),
                   {MakeArg(MakeIdentifierExpr("missing"))});
  auto out_region_ctrl = cursive0::eval::EvalSigma(ctx, *call_region_ctrl, sigma);
...
void TestEvalRecvAndArmBodyRules() {
  ...
  cursive0::eval::Binding recv_binding;
  BindVal(sigma, "recv", cursive0::eval::Value{cursive0::eval::UnitVal{}},
          BindInfo{}, &recv_binding);
  sigma.store[recv_binding.addr] = cursive0::eval::Value{cursive0::eval::UnitVal{}};

  auto move_recv = MakeMethodCallExpr(
      MakeMoveExpr(MakeIdentifierExpr("recv")), "noop", {});
  auto out_move = cursive0::eval::EvalSigma(ctx, *move_recv, sigma);
  assert(IsCtrl(out_move) || IsVal(out_move));

  auto move_ctrl = MakeMethodCallExpr(
      MakeMoveExpr(MakeIdentifierExpr("missing")), "noop", {});
  auto out_move_ctrl = cursive0::eval::EvalSigma(ctx, *move_ctrl, sigma);
  assert(IsCtrl(out_move_ctrl));

  auto ref_recv = MakeMethodCallExpr(MakeIdentifierExpr("recv"), "noop", {});
  auto out_ref = cursive0::eval::EvalSigma(ctx, *ref_recv, sigma);
  assert(IsCtrl(out_ref) || IsVal(out_ref));

  auto ref_ctrl = MakeMethodCallExpr(MakeIdentifierExpr("missing"), "noop", {});
  auto out_ref_ctrl = cursive0::eval::EvalSigma(ctx, *ref_ctrl, sigma);
  assert(IsCtrl(out_ref_ctrl));

  // Dynamic receiver (active)
  cursive0::eval::Binding dyn_binding;
  BindVal(sigma, "dyn", cursive0::eval::Value{cursive0::eval::UnitVal{}},
          BindInfo{}, &dyn_binding);
  const auto dyn_addr = cursive0::eval::AllocAddr(sigma);
  const auto scope_id = cursive0::eval::CurrentScopeId(sigma);
  cursive0::eval::DynamicVal dyn_val;
  dyn_val.class_path = {"Unknown"};
  dyn_val.data.qual = cursive0::analysis::RawPtrQual::Imm;
  dyn_val.data.addr = dyn_addr;
  dyn_val.concrete_type = nullptr;
  sigma.addr_tags[dyn_addr] =
      cursive0::eval::RuntimeTag{cursive0::eval::RuntimeTagKind::ScopeTag, scope_id};
  sigma.store[dyn_binding.addr] = cursive0::eval::Value{dyn_val};

  auto dyn_recv = MakeMethodCallExpr(MakeIdentifierExpr("dyn"), "noop", {});
  auto out_dyn = cursive0::eval::EvalSigma(ctx, *dyn_recv, sigma);
  assert(IsCtrl(out_dyn) || IsVal(out_dyn));

  // Dynamic receiver (expired)
  cursive0::eval::Binding dead_binding;
  BindVal(sigma, "dyn_dead", cursive0::eval::Value{cursive0::eval::UnitVal{}},
          BindInfo{}, &dead_binding);
  const auto dead_addr = cursive0::eval::AllocAddr(sigma);
  cursive0::eval::DynamicVal dead_val;
  dead_val.class_path = {"Unknown"};
  dead_val.data.qual = cursive0::analysis::RawPtrQual::Imm;
  dead_val.data.addr = dead_addr;
  dead_val.concrete_type = nullptr;
  sigma.addr_tags[dead_addr] =
      cursive0::eval::RuntimeTag{cursive0::eval::RuntimeTagKind::ScopeTag, 9999};
  sigma.store[dead_binding.addr] = cursive0::eval::Value{dead_val};

  auto dead_recv = MakeMethodCallExpr(MakeIdentifierExpr("dyn_dead"), "noop", {});
  auto out_dead = cursive0::eval::EvalSigma(ctx, *dead_recv, sigma);
  assert(IsCtrl(out_dead));

  auto block_expr = MakeBlockExpr(MakeBlockWithTail(lit));
  auto out_block = cursive0::eval::EvalSigma(ctx, *block_expr, sigma);
  ...
  auto out_match_block = cursive0::eval::EvalSigma(ctx, *match_block_expr, sigma);
  ...
  auto out_match_expr = cursive0::eval::EvalSigma(ctx, *match_expr_expr, sigma);
