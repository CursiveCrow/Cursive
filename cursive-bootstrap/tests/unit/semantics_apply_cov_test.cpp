...
using cursive0::semantics::BindingValue;
...
using cursive0::semantics::MethodTarget;
...
void TestApplyProcSigma() {
  ...
  Sigma sigma;
  Outcome out = cursive0::semantics::ApplyProcSigma(ctx, proc, {}, sigma);
  assert(IsVal(out));
...
void TestApplyRecordCtorSigma() {
  ...
  SetExprType(expr_types, lit_x, cursive0::sema::MakeTypePrim("i32"));
  SetExprType(expr_types, lit_y, cursive0::sema::MakeTypePrim("i32"));
  ...
  sema_ctx.sigma.types[cursive0::sema::PathKeyOf({"Point"})] = record;
  ...
  Sigma sigma;
  Outcome out =
      cursive0::semantics::ApplyRecordCtorSigma(ctx, {"Point"}, sigma);
  assert(IsVal(out));
  const auto& value = std::get<Value>(out.node);
  const auto* record_val = std::get_if<cursive0::semantics::RecordVal>(&value.node);
  assert(record_val);
...
void TestApplyRecordCtorSigmaCtrl() {
  ...
  record.span = EmptySpan();
  record.doc = {};
  sema_ctx.sigma.types[cursive0::sema::PathKeyOf({"Bad"})] = record;
  ...
  Sigma sigma;
  Outcome out =
      cursive0::semantics::ApplyRecordCtorSigma(ctx, {"Bad"}, sigma);
  assert(IsCtrl(out));
}
...
void TestApplyMethodSigma() {
  ...
  Value self_value{cursive0::semantics::UnitVal{}};
  BindingValue self_arg = self_value;

  Sigma sigma;
  Outcome out = cursive0::semantics::ApplyMethodSigma(
