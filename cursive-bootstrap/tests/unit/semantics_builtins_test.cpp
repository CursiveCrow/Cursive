...
Value StringViewValue(std::string_view text) {
  StringVal out;
  out.state = cursive0::sema::StringState::View;
...
Value BytesViewValue(const std::vector<std::uint8_t>& bytes) {
  BytesVal out;
  out.state = cursive0::sema::BytesState::View;
  out.bytes = bytes;
  return Value{out};
}
...
std::optional<std::vector<std::uint8_t>> ManagedBytes(const Value& value) {
  ...
  if (!bytes || bytes->state != cursive0::sema::BytesState::Managed) {
    return std::nullopt;
  }
  return bytes->bytes;
}
...
std::optional<std::vector<std::uint8_t>> ManagedStringBytes(const Value& value) {
  ...
  if (!str || str->state != cursive0::sema::StringState::Managed) {
    return std::nullopt;
  }
  return str->bytes;
}
...
int main() {
  ...
  RawPtrVal heap_ptr;
  heap_ptr.qual = cursive0::sema::RawPtrQual::Imm;
  ...
  assert(raw);
  assert(raw->qual == cursive0::sema::RawPtrQual::Mut);
  ...
  RecordVal sys;
  sys.record_type = cursive0::sema::MakeTypePath({"System"});
  Value sys_value{sys};

  StringVal key;
  key.state = cursive0::sema::StringState::View;
  key.bytes = {0x46};
  Value key_value{key};
  ...
  RawPtrVal fs_ptr;
  fs_ptr.qual = cursive0::sema::RawPtrQual::Imm;
  fs_ptr.addr = fs_addr;
  ...
  assert(name1_val && name1_val->state == cursive0::sema::StringState::Managed);
  ...
  assert(name2_val && name2_val->state == cursive0::sema::StringState::Managed);
  ...
  assert(name3_val && name3_val->state == cursive0::sema::StringState::Managed);
  ...
  assert(name4_val && name4_val->state == cursive0::sema::StringState::Managed);
...

  {
    ...
    const sema::TypePath region_path = {"Region"};
    Value opts{UnitVal{}};
