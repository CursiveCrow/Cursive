    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            if mode == "ir":
                ...
                if output_pipeline is not True:
            ...
            if mode == "ir":
                ...
                expect_path = test_path.with_suffix(".expect.ll")
                expect_path.write_text(normalized + "\n", encoding="utf-8")
            else:
                try:
                ...
                expect_path = test_path.with_suffix(test_path.suffix + ".expect.json")
                expect_path.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
            return True, ""
        finally:
...
def main() -> int:
    ...
    if failures:
        print(f"{failures} expect file(s) failed to regenerate")
        return 1
