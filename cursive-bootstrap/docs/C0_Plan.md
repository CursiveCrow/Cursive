...
* The compiler pipeline SHOULD be single-threaded in `cursivec0`. If any phase is parallelized, diagnostic emission order MUST still match the spec’s `Emit` order and all outputs MUST remain deterministic.
...
* Diagnostics MUST preserve the order of `Emit` calls (spec §1.6.3 `Emit-Append` + §1.6.5 `Order` is identity): do not sort diagnostics by span. Determinism is achieved by ensuring traversal and emission are deterministic.
...
* Diagnostics must use the spec’s **DiagId → Code** mapping (generated from the spec tables), and must carry stable spans where defined.
...
1. Implement tasks in the order listed in §9 (sequencing), keeping the compiler runnable end-to-end as early as possible.
2. After each task:
...
* **Fallible operations:** Filesystem I/O, UTF-8 decoding, tool resolution, process invocation, and linking MUST return explicit errors (see §0.3). Exceptions MUST NOT cross subsystem boundaries.
* **Span fidelity:** Every token and AST node that corresponds to source text MUST carry a stable `Span` (or a reference to one) suitable for diagnostics, per §1.6.
...
1. **Phase 0 (Project):** `LoadProject(project_root)` → `ProjectContext`
...
2. **Phase 1 (Source → AST):** For each module in deterministic order:

   * read bytes → UTF-8 decode + BOM handling → normalize line endings → logical line map
   * lex → parse (with recovery) → `ASTModule`

   Output: `ASTProgram` (project-wide module map + module list)

3. **Phase 2 (Compile-time execution):** **deferred** in `cursivec0`

   * detect compile-time execution forms and reject via the spec’s `UnsupportedForm` relationship

4. **Phase 3 (Sema):** `ASTProgram` → `TypedProgram`
...
5. **Phase 4 (Codegen):** `TypedProgram` → `Object(s)` → `exe`

   * lower to the internal IR model (as specified in §6.0)
   * IR checks and panic instrumentation
   * emit LLVM IR (text and/or bitcode as per output plan)
   * emit object(s) and link using `lld-link` per §2.5

**Output gating rule of thumb (driver-level):**

* The driver MUST follow the spec’s output rules (especially `Out-*` and `Output-Pipeline-*`).
...
Use a small number of long-lived “context” objects to make ownership, caching, and determinism explicit:
...
These are not “extra features”; they are implementation patterns that reduce risk and increase spec compliance.

**Recommended patterns:**

* **Two-tier IR model:** Keep the spec’s internal IR model (§6.0) as the unit under test, then translate to LLVM IR. This makes rule-to-code mapping tractable.
...
* Sorting diagnostics “by location” or “by severity” (violates spec `Order` identity).
* Using pointer addresses or hash map iteration to break ties (non-deterministic).
* Performing tool resolution via build-system discovery (must follow §2.6 at runtime).
...
1. **One task at a time:** implement the task’s spec rules, add `SPEC_RULE`, add `SPEC_COV`, and land tests before moving to the next task.
2. **No “helpful” extensions:** do not add language features, flags, or optimizations not in the pinned spec.
3. **Keep the compiler runnable early:** always preserve the ability to run at least Phase 0 + Phase 1 on trivial projects, even if later phases are stubbed behind “unsupported” diagnostics.
...
* Emits diagnostics with stable **DiagId → Code** mapping and correct ordering.
* Implements **every rule in the Cursive0 specification** (including “reject/unsupported” rules).
* Ships with tests demonstrating that **each implemented rule is tested**.

### Hard constraints derived from the spec

* **Toolchain**: MUST resolve `llvm-as` and `lld-link` at compiler runtime using §2.6 `SearchDirs(P)` **exactly**:
  ...
  * Else: `SearchDirs(P) = PATHDirs` (ordered list of PATH directories).
  ...
  Tool resolution MUST follow §2.6 `ResolveTool-*` and MUST fail if the tool is not present in `SearchDirs(P)`. When multiple directories are present (PATH case), resolve by scanning directories in list order and selecting the first match. IR assembly for `emit_ir="bc"` MUST follow §2.6 `AssembleIR-*` (including `ResolveTool(llvm-as)` and `AssembleIR`).

* **Target**: MUST target Win64 with target triple `x86_64-pc-windows-msvc` and the LLVM module header constants specified in §1.5 and §6.12.1.

* **Project manifest**: MUST load the project from `${project_root}/Cursive.toml` per §2.1 `ParseManifest`/`ValidateManifest` (host filesystem semantics).

* **Source files**: module discovery MUST treat only files with extension `.cursive` as Cursive0 source files (§2.4 `Files(d)` / `CompilationUnit(d)` / `Module-Dir`).

* **Runtime library**: MUST locate the runtime library at `${project_root}/runtime/cursive0_rt.lib` and MUST validate required symbols during output planning and linking (§2.5 `ResolveRuntimeLib-*` and `Link-*`).

* **Phase 2 is deferred**: compile-time execution forms MUST be rejected as unsupported (see §4 and the `UnsupportedForm` relationship).
...
cursivec0/
  ...
  llvm/                          # optional: vendored LLVM tools per §2.6 SearchDirs
    llvm-21.1.8-x86_64/
      bin/
        llvm-as(.exe)
        lld-link(.exe)
  ...
  tools/
    ...
    diag_extractor.py           # extracts diag tables + DiagId→Code mapping
  ...
  include/
    cursive0/
      ...
      codegen/
        ir_model.h               # internal IR layer (as per §6.0)
        layout.h
        abi.h
        mangle.h
        lower_expr.h
      ...
      runtime/
        runtime_interface.h      # declarations consistent with §6.9 / §6.12.6
...
* All compiler “judgments” (Parse*, Resolve*, T-*, Lower-*, Exec*, Eval*) map to:
...
## 3. Traceability: enforcing “every rule implemented and tested”
...
* `SPEC_DEF("DefinitionName", "§X.Y.Z")` for non-rule but normative definitions.
...
* NOTE: build-time discovery of LLVM libraries (to build `cursivec0`) is separate from runtime resolution of `llvm-as`/`lld-link` for compiling user projects; the latter MUST follow §2.6 (do not reuse CMake `find_program` results for compiler runtime behavior).
...
**Spec:** §1.6.1 rules `WF-Location`, `WF-Span`, `Span-Of`.
**Implement:**
...
**Spec:** §1.6.2 rules `No-Unknown-Ok`, `Attach-Token-Ok`, `Attach-Tokens-Ok`.
**Implement:**
...
**Completion notes (2026-01-04):**
* Implemented `TokenKind`, `RawToken`, `Token`, `EofToken`, `AttachSpan(s)`, `NoUnknownOk`, and `MakeEofToken` in `include/cursive0/syntax/token.h` and `src/syntax/token.cpp` with `SPEC_RULE`/`SPEC_DEF` anchors; EOF modeled as a separate sentinel (not a `TokenKind`) per §3.2.1/§3.2.4.
...
**Completion notes (2026-01-04):**
* Implemented diagnostic data model (`Severity`, `Diagnostic`, `DiagnosticStream`), `Emit`, `HasError`, and `CompileStatus` in `include/cursive0/core/diagnostics.h` and `src/core/diagnostics.cpp` with `SPEC_RULE`/`SPEC_DEF` anchors for §1.6.3.
...
**Spec:** §1.6.4 rules `Code-Spec`, `Code-C0`.
**Implement:**
...
**Spec:** §1.6.5 rule `Order`; §1.6.6 rendering requirements; §1.6.7 rule `NoSpan-External`.
**Implement:**

* `include/cursive0/core/diagnostic_render.h`
* `src/core/diagnostic_render.cpp`

* Implement `Order(Δ)` as **identity**: return diagnostics in the same sequence they were appended. Do not perform any secondary sorting or bucketing.
**Tests:**
...
* Implemented identity ordering and exact render formatting per §1.6.6, and enforced span stripping for external diagnostics.
...
**Spec:** §8 (Appendix A - Diagnostic Codes); all `E-*` and `W-*` diagnostic code tables.
**Implement:**
...
* Message content for each diagnostic code in §8.0–§8.13:
...
* `SPEC_DEF("DiagId-Code-Map", "§8.0")` and related coverage for each diagnostic category.
...
**Spec:** §1.1 rule `Reject-IllFormed`.
**Implement:**
...
**Completion notes (2026-01-04):**
* Added conformance API (`WF`, `C0Conforming`, `RejectIllFormed`) in `include/cursive0/sema/conformance.h` and `src/sema/conformance.cpp` with `SPEC_RULE("Reject-IllFormed")` and §1.1 `SPEC_DEF` anchors.
...
**Spec:** §1.1.1 rule `Perm-Shared-Unsupported`; PermSet_C0 definitions; unsupported constructs set.
**Implement:**
...
**Spec:** §1.2 rules `Static-Undefined`, `Static-Undefined-NoCode`, `Dynamic-Undefined-UVB`.
**Implement:**
...
* Compile-test coverage for `Static-Undefined` rules deferred: no literal `⊥` premises exist in the Cursive0 spec, so there is no compile-path trigger; unit coverage is the correct substitute until the spec adds such a rule.
* Build/test: `cmake --build cursive-bootstrap/build`, `ctest --test-dir cursive-bootstrap/build` (clock-skew warnings from gmake observed; tests passed).

#### T-CORE-009 Unsupported constructs policy

**Spec:** §1.4 rules `WF-Attr-Unsupported`, `Unsupported-Construct`.
**Implement:**
...
* **Incomplete (as of 2026-01-04):** parser-level implementations required by §3.3.6.14 (e.g., `Parse-Attribute-Unsupported`, `Parse-Generic-Header-Unsupported`, `Parse-Comptime-Unsupported`, etc.) were pending under **T-PARSE-004**. The stop-gap implementation emitted diagnostics without spans and did not cover all UnsupportedForm cases.

**Completion notes (2026-01-06):**
* Parser-level unsupported-construct parsing rules are now implemented in Phase 1 under **T-PARSE-004** (including `Parse-Attribute-Unsupported`, `Parse-Generic-Header-Unsupported`, `Parse-Comptime-Unsupported`, and the remaining §3.3.6.14 rules), completing CORE-009; see T-PARSE-004 completion notes for file/test details.

#### T-CORE-010 Host primitives interface

**Spec:** §1.7 definitions `HostPrim`, `HostPrimDiag`, `HostPrimRuntime`, `MapsToDiagOrRuntime`, `HostPrimFail`.
**Implement:**
...
* `src/core/host_primitives.cpp`
  ...
  * Implement `HostPrimRuntime` subset (primitives used by runtime: `FSPrim ∪ FilePrim ∪ DirPrim`).
  * Ensure `HostPrimFail(p)` for any `p ∉ HostPrimDiag ∪ HostPrimRuntime` results in `IllFormed(p)`.
...
* `SPEC_DEF("HostPrim", "§1.7")`, `SPEC_DEF("HostPrimDiag", "§1.7")`, `SPEC_DEF("HostPrimRuntime", "§1.7")`.

**Completion notes (2026-01-04):**
* Added `include/cursive0/core/host_primitives.h` and `src/core/host_primitives.cpp` with `HostPrim` enum, classification helpers, `HostPrimFail`, and `HostPrimFailureIllFormed` guard, plus `SPEC_DEF` anchors for §1.7.
* Implemented FS/File/Dir primitive classification derived from §7.7; marked derivation with `[AMBIGUOUS: Source text unclear. See original section 1.7.]` and `[TODO: Content missing from source draft.]` per spec integrity rules.
...
**Spec:** §2.1 rules `Parse-Manifest-Ok`, `Parse-Manifest-Missing`, `Parse-Manifest-Err`.
**Implement:**

* MUST parse `${project_root}/Cursive.toml` as defined in §2.1 (host filesystem semantics).
...
**Spec:** §2.1 rules `ValidateManifest-Ok`, `ValidateManifest-Err`, `WF-TopKeys`, `WF-TopKeys-Err`, `WF-RelPath`, `WF-RelPath-Err`, `WF-Project-Root`, `Resolve-Canonical`, `Resolve-Canonical-Err`, plus all `WF-Assembly-*` rules in §2.1.
**Implement:**
...
* Ensure each `WF-*` rule in §2.1 gets at least one failing and/or passing test with `SPEC_COV`.
...
**Spec:** §2.1 rules `Step-Discover`, `Step-Discover-Err-Root`, `Step-Discover-Err-Modules`, `Step-Parse`, `Step-Parse-Err`, `Step-Resolve`, `Step-Resolve-Err`, `Step-Validate`, `Step-Validate-Err`, `LoadProject-Ok`, `LoadProject-Err`.
**Implement:**
...
* **Mock/placeholder wiring (for early bring-up before T-PROJ-006/007 land):**
  ...
  * Placeholder implementations (if needed) MUST be clearly marked with TODO comments and MUST NOT add `SPEC_RULE` anchors for §2.4 or §2.5 rules. They should return deterministic, trivial outputs (e.g., empty module list + no diags; OutputPaths computed per §2.5 formulas).
...
* Added minimal placeholder interfaces for module discovery/output paths to unblock the pipeline:
  ...
  * `include/cursive0/project/outputs.h` + `src/project/outputs.cpp` (`ComputeOutputPaths` per §2.5 formulas)
...
**Spec:** §2.2 rules `WF-Assembly-Count`, `WF-Assembly-Count-Err`.
**Implement:**
...
* Apply `WF-Assembly-Count` only when `A` is defined:
  * `|A| = 1` → `WF-Assembly-Count` holds.
  * `|A| ≠ 1` → `WF-Assembly-Count-Err` and emit `E-PRJ-0202`.
* In array-of-tables cases, emit only a single diagnostic `E-PRJ-0202`. If `|A| ≠ 1`, record both `SPEC_RULE("WF-Assembly-Table-Array")` and `SPEC_RULE("WF-Assembly-Count-Err")` in the same failure path to satisfy rule coverage without emitting multiple diagnostics.
...
* Implemented assembly count handling in `src/project/project_validate.cpp`:
  ...
  * `WF-Assembly-Count-Err` recorded on array-of-tables with size ≠ 1 while emitting a single `E-PRJ-0202` (shared with `WF-Assembly-Table-Array`).
* Added compile tests:
...
**Spec:** §2.3.1 rule `FileOrder-Rel-Fail`; §2.3.3 rules `DirSeq-Rel-Fail`, `DirSeq-Read-Err`.
**Implement:**
...
**Spec:** §2.4 rules `Disc-Start`, `Disc-Add`, `Disc-Skip`, `Disc-Collision`, `Disc-Rel-Fail`, `Disc-Invalid-Component`, `Disc-Done`, `CompilationUnit-Rel-Fail`, `Module-Dir`, `Module-Path-Root`, `Module-Path-Rel`, `Module-Path-Rel-Fail`, `Modules-Ok`, `Modules-Err`, `WF-Source-Root`, `WF-Source-Root-Err`, `WF-Module-Path-Ok`, `WF-Module-Path-Ident-Err`, `WF-Module-Path-Reserved`, `WF-Module-Path-Collision`.
**Implement:**
...
* **Interface requirements for T-PROJ-003:**
  * Provide `ModuleInfo` with at least:
    * `path`: module path string used by `prec_mod` ordering in §2.5.
...
**Completion notes (2026-01-05):**
* Implemented §2.4 module discovery in `include/cursive0/project/module_discovery.h` and `src/project/module_discovery.cpp`:
...
* Spec clarification in `docs/Cursive0.md`: defined `AsmName(Γ)` and `Dirs(S)` (recursive, includes `S`).
* Tests:
...
**Spec:** §2.5 rules `Out-Start`, `Output-Pipeline`, `Output-Pipeline-Err`, `Out-Obj-*`, `Out-IR-*`, `Out-Link-*`, `ModuleList-*`, `ResolveRuntimeLib-*`, `Link-*`, `Emit-IR-*`, `Emit-Objects-*`, `CodegenIR-LLVM`, `CodegenObj-LLVM`. 
**Implement:**

* Link invocation MUST match §2.5 `LinkFlags`/`LinkArgsOk` exactly (including `/OUT:…`, `/ENTRY:main`, `/SUBSYSTEM:CONSOLE`, `/NODEFAULTLIB`).
* **Runtime compatibility preflight:** implement §2.5 `LinkerSyms` / `MissingRuntimeSym` by inspecting the *link inputs* (Objs + `cursive0_rt.lib`) and verifying `RuntimeRequiredSyms (= RuntimeSyms)` are present **before** invoking `lld-link`. Implement symbol inspection in-process via LLVM Object (`llvm::object::COFFObjectFile` / `llvm::object::Archive`) so the driver can reliably emit `Out-Link-Runtime-Incompatible` instead of a generic `Out-Link-Fail`.
...
* **Interface requirements for T-PROJ-003:**
  * Define `struct OutputPaths { root, obj_dir, ir_dir, bin_dir }` and `ComputeOutputPaths(project_root, asm)` that implements the §2.5 `OutputPaths` equations.
  * `LoadProject` will populate `Project.outputs` using `ComputeOutputPaths` and MUST NOT perform output hygiene checks here (those are part of §2.5 pipeline).
**Tests:**
* Output collision tests: two modules with colliding mangled object names (`Out-Obj-Collision`)
* IR emission tests for `emit_ir = "ll"` and `emit_ir = "bc"` and “none”
...
* Added `ModuleList-Err`/`ModuleList-Ok` anchors in `src/project/load_project.cpp` to satisfy §2.5 ModuleList rules.
* Tests:
...
**Spec:** §2.6 rules `ResolveTool-Ok`, `ResolveTool-Err-Linker`, `ResolveTool-Err-IR`, `AssembleIR-Ok`, `AssembleIR-Err`. 
...
**Spec:** §2.7 rule `WF-Unwind-Unsupported`.
**Implement:**
...
**Spec:** §3.1.3 rules `Decode-Ok`, `Decode-Err`, `StripBOM-Start`, `StripBOM-None`, `StripBOM-Empty`, `StripBOM-Embedded`.
**Implement:**
...
* `SPEC_COV("<RuleId>")` for each §3.1.3 rule.
...
**Spec:** §3.1.4 rules `Norm-Empty`, `Norm-CRLF`, `Norm-CR`, `Norm-LF`, `Norm-Other`.
**Implement:**

* `src/core/source_load.cpp`
**Tests:**
* Unit tests confirming span/line mapping after normalization
* `SPEC_COV("<RuleId>")` for each §3.1.4 rule.
...
* Tests:
  * Added `tests/unit/core_line_normalization_test.cpp` covering normalization output and line/column mapping with `SPEC_COV` for each §3.1.4 rule.
...
**Spec:** §3.1.5 rule `WF-Prohibited` (used by §3.1.8 `Step-Prohibited`/`Step-Prohibited-Err`).
**Implement:**
...
**Spec:** §3.1.7 rule `Missing-Terminator-Err` (plus line-continuation definitions in §3.1.7). 
**Implement:**
...
**Spec:** §3.1.8 rules `Step-Size`, `Step-Decode`, `Step-Decode-Err`, `Step-BOM`, `Step-Norm`, `Step-EmbeddedBOM-Err`, `Step-LineMap`, `Step-Prohibited`, `Step-Prohibited-Err`, `LoadSource-Ok`, `LoadSource-Err`.
**Implement:**
...
* Addendum (diagnostics): map `Step-Decode-Err` → `E-SRC-0101` (NoSpan-Decode); `Step-EmbeddedBOM-Err` → `E-SRC-0103`; `Step-Prohibited-Err` → `E-SRC-0104` (forbidden control character).
* Addendum (ReadBytes boundary): `ReadBytes-Err` → `E-SRC-0102` is emitted by callers (e.g., `ParseUnitSources`/`ParseModule`) before `LoadSource`; `LoadSource` itself assumes bytes are already provided.
...
**Completion notes (2026-01-05):**
* Implemented `LoadSource` pipeline in `src/core/source_load.cpp` with `SPEC_RULE` anchors for §3.1.8 steps and `LoadSource-*`, plus diagnostic mapping to `E-SRC-0101/0103/0104` (spans deferred to T-SRC-006).
* Added `include/cursive0/core/source_load.h` with `SourceLoadResult` and `LoadSource` API.
* Tests:
  * Extended `tests/unit/core_source_load_test.cpp` with `SPEC_COV` coverage for all §3.1.8 rules plus success/error cases.
...
**Spec:** §3.1.9 rules `NoSpan-Decode`, `Span-BOM-Warn`, `Span-BOM-Embedded`, `Span-Prohibited`.
**Implement:**

* `src/core/source_load.cpp`
* Addendum (span helpers): use temporary `SourceFile` data per §3.1.9 (`S_tmp`, `Utf8Offsets`, `SpanOf`) to compute BOM/prohibited spans without relying on later normalization steps.
...
**Completion notes (2026-01-05):**
* Updated `cursive-bootstrap/docs/Cursive0.md` §3.1.9 to compute embedded-BOM spans against normalized `T` and to require emitting `W-SRC-0101` even when `LoadSource` later fails.
...
* Tests:
  * Extended `tests/unit/core_source_load_test.cpp` with `SPEC_COV` for §3.1.9 rules and explicit span assertions for BOM warning, embedded BOM error, prohibited code point error, and decode error (no span).
* Build/test: `cmake --build cursive-bootstrap/build`; `ctest --test-dir cursive-bootstrap/build --output-on-failure` (25/25 passed; gmake clock-skew warnings observed).
...
**Spec:** §3.2.5 rules `Scan-Line-Comment`, `Doc-Comment`, `Block-Start`, `Block-Step`, `Block-End`, `Block-Done`, `Block-Comment-Unterminated`.
**Implement:**

* `include/cursive0/syntax/lexer.h`
* `src/syntax/lexer_ws.cpp`
* Addendum (newline suppression): emit `ScalarRange` spans for line/block comments and pass them to `LexNewlines` so LF inside comments does not produce `Newline` tokens (§3.2.1 `LexNewline`, `LexNoComments`).
...
* Tests:
  * Added `tests/unit/syntax_lexer_ws_test.cpp` with `SPEC_COV` coverage for §3.2.5 rules, doc comment body/span checks, nested block comments, unterminated block comment diagnostics, and newline suppression behavior.
...
**Spec:** §3.2.6 rules `Lex-Int`, `Lex-Float`, `Lex-Numeric-Err`, `Lex-String`, `Lex-String-Unterminated`, `Lex-String-BadEscape`, `Lex-Char`, `Lex-Char-Unterminated`, `Lex-Char-BadEscape`, `Lex-Char-Invalid`.
**Implement:**

* `src/syntax/lexer_literals.cpp`
* Addendum (newline suppression): emit `ScalarRange` spans for string/char literals and pass them to `LexNewlines` so LF inside literals does not produce `Newline` tokens (§3.2.1 `LexNewline`, `LexNoComments`).
...
* Tests:
  * Added `tests/unit/syntax_lexer_literals_test.cpp` with `SPEC_COV` for all §3.2.6 literal rules, including bad escapes, malformed numeric underscores, leading-zero warnings, and unterminated recovery behavior.
...
**Spec:** §3.2.7 rules `Lex-Identifier`, `Lex-Ident-InvalidUnicode`, `Lex-Ident-Token` (keywords/reserved lexemes per §3.2.3).
**Implement:**
...
**Completion notes (2026-01-05):**
* Added Unicode identifier helpers in `src/core/unicode.cpp` and `include/cursive0/core/unicode.h` for XID start/continue, ident start/continue, and non-character checks per §3.2.2.
* Implemented identifier scanning and classification in `src/syntax/lexer_ident.cpp`, including `E-SRC-0307` emission for first non-character and Bool/Null/Keyword/Identifier classification per §3.2.7.
* Added Cursive0 keyword table in `src/syntax/keyword_table.inc` (Cursive0 §3.2.3 reserved list only; `in` remains identifier).
...
* Tests: added `tests/unit/syntax_lexer_ident_kw_test.cpp` with `SPEC_COV` for all §3.2.7 rules, including contextual `in`, non-reserved future keywords, Unicode XID sample, and non-character diagnostic span checks.
...
**Spec:** §3.2.9 rules `Max-Munch`, `Max-Munch-Err`.
**Implement:**
...
**Completion notes (2026-01-05):**
* Added `NextTokenResult` + `NextToken` API in `include/cursive0/syntax/lexer.h`, implementing §3.2.9 `Max-Munch`/`Max-Munch-Err`.
...
**Spec:** §3.2.10 rules `LexSecure-Err`, `LexSecure-Warn` (lexically sensitive enforcement per §3.1.2 and §3.2.10).
**Implement:**
...
**Completion notes (2026-01-05):**
* Added `IsSensitive` in `src/core/unicode.cpp` with declaration in `include/cursive0/core/unicode.h` (spec defs for §3.2.2).
...
**Spec:** §3.2.11 rules `Lex-Start`, `Lex-End`, `Lex-Whitespace`, `Lex-Newline`, `Lex-Line-Comment`, `Lex-Doc-Comment`, `Lex-Block-Comment`, `Lex-String-Unterminated-Recover`, `Lex-Char-Unterminated-Recover`, `Lex-Sensitive`, `Lex-Token`, `Lex-Token-Err`; §3.2.12 rules `Tokenize-Ok`, `Tokenize-Secure-Err`, `Tokenize-Err` (and §3.2.12 definitions like Filter).
**Implement:**
...
* Implemented small-step tokenization and big-step `Tokenize` in `src/syntax/tokenize.cpp`, including block-comment failure → `LexError` (`E-SRC-0306`), unterminated string/char recovery that resumes at LF/EOF without `E-SRC-0309`, `SensitiveTok` handling, and `LexSecure` gating for `Tokenize-Ok` vs `Tokenize-Secure-Err`.
...
**Spec:** §3.3.1 definitions (e.g., ParseFileOk/ParseFileBestEffort) and §3.3.6 rules `ParseFile-Ok`, `ParseItems-Empty`, `ParseItems-Cons`, `Parse-Item-Err` (parser is best-effort; errors are represented in items + diagnostics).
**Implement:**
...
* **Module docs (`MDoc`)**: For `ParseFile`, compute `MDoc = ModuleDocs(D)` using the lexer doc stream `D` (§3.3.11). Item doc attachment is **not** part of `ParseItems` and is handled by **T-PARSE-009**.
* **Diagnostics separation and order**: `P_0` starts with an **empty** parser diagnostic stream (`Δ = []`) per §3.3.6. `DiagStream(P_1)` contains **only parser diagnostics**. Lexer/tokenize diagnostics are produced by `Tokenize(S)` and must be emitted **before** parse diagnostics when constructing final output. If `Tokenize` fails, parsing does not run and only lexer diagnostics are reported.
...
**Spec:** §3.3.4 definitions (including UsingKeyword) and rule `Method-Context-Err` (plus any other bold rules in §3.3.4).
**Implement:**
...
**Stubbing note:** `Method-Context-Err` depends on method/record/class ASTs and was stubbed until T-PARSE-004 (now implemented; see completion notes); `TypeArgsUnsupported`/`C0TypeRestricted` depend on type parsing and will be stubbed until T-PARSE-005. Add explicit “unstub” items to those tasks.
...
**Spec:** §3.3.5 rules `Tok-Consume-Kind`, `Tok-Consume-Keyword`, `Tok-Consume-Operator`, `Tok-Consume-Punct`, `List-Start`, `List-Cons`, `List-Done`, `Trailing-Comma-Err`.
**Implement:**
...
* The parser MUST NOT “globally allow trailing commas”. Trailing commas are handled **per-list** by the spec rules.
...
* §3.3.6.14 “Generic Syntax Rejection” helpers: `AngleDelta`, `AngleStep`, `AngleScan`, `SkipAngles` (used by the generic rejection rules).
...
* `src/syntax/parser_angle.cpp` (or `src/syntax/parser_util.cpp`) with **exact** helper behavior:
  ...
  * `AngleScan(P0, P, d)` and `SkipAngles(P)` MUST follow the spec’s recursion/termination behavior (including EOF behavior).

**LLM handoff warning (do not “simplify”):**

* These helpers are **semantic glue** between the lexer’s maximal-munch tokenization and the parser rules. If they are missing or approximate, nested `Ptr<…>` types, generic-rejection sync, and some `transmute` parses will diverge from the spec.
...
* §3.3.6.1 identifiers/paths rules `Parse-Path-*`
* §3.3.6.2 visibility rules `Parse-Vis-*`
* §3.3.6.3 using rules `Parse-Using-*`
* §3.3.6.4 static decl, §3.3.6.5 proc decl, §3.3.6.6 record, §3.3.6.7 enum, §3.3.6.8 modal, §3.3.6.9 implements, §3.3.6.10 class, §3.3.6.11 type alias
* §3.3.6.13 helper parsing rules (76 rules)
* §3.3.6.14 unsupported constructs parsing rules (8 rules)

**Required explicit coverage (do not treat as “implied”):**
...
* Unsupported construct parsing rules MUST be implemented **by name** (they are not optional “subset gates”):
...
**Completion notes (2026-01-06):**
* Implemented §3.3.6 item + helper parsing in `src/syntax/parser_items.cpp` (using/static/procedure/record/enum/modal/class/type alias items, record/class/state members, list/tail helpers, return-at-module error, and item-level recovery) with `SPEC_RULE` anchors for all listed Parse-* rules.
* Implemented §3.3.6.1–.2 path + visibility parsing in `src/syntax/parser_paths.cpp`, including a spec-correct `ParseQualifiedHead` that reuses `ParseModulePathTail` without extra `Advance`.
...
* Tests:
  * Added `tests/unit/syntax_parser_items_test.cpp` with comprehensive `SPEC_COV` coverage for §3.3.6 rules and helper rules; registered in `tests/CMakeLists.txt`.
...
**Spec:** §3.3.7 and §3.3.7.1 rules (23 + 25).

**Required explicit coverage (do not treat as “handled by maximal munch”):**
...
**Completion notes (2026-01-06):**
* Implemented §3.3.7/§3.3.7.1 type parsing in `src/syntax/parser_types.cpp`, including union tails, tuple/param list parsing, Ptr shift-splitting, array/slice, raw pointers, string/bytes/ptr state, dynamic/modal state, and type paths with generic rejection (`SkipAngles` + `SyncType`) per `Parse-Type-Generic-Unsupported`.
...
* Build/test: `cmake --build /mnt/c/Dev/Cursive/cursive-bootstrap/build`; `ctest --test-dir /mnt/c/Dev/Cursive/cursive-bootstrap/build --output-on-failure` (36/36 passed; clock-skew warnings observed during build).
* Update (2026-01-06): replaced the placeholder span-only Type nodes with a full Type AST per §3.3.7 (perm/union/func/tuple/array/slice/ptr/raw/string/bytes/dynamic/modal/path) in `include/cursive0/syntax/ast.h`, and updated `parser_types.cpp` to construct those nodes (including `Parse-Type-Err` base `TypePrim("!")` + perm wrapping).

#### T-PARSE-006 Expression parsing and precedence

**Spec:** §3.3.8 + subparts §3.3.8.1–§3.3.8.10 rules (range, bin chains, power, cast, unary, postfix, primary, helper rules).
...
**Completion notes (2026-01-06):**
* Implemented §3.3.8 expression parsing in `src/syntax/parser_expr.cpp` (range, left-chains, power, cast, unary, postfix, primary, match/if/loop/else helpers), including `Parse-Transmute-Expr` and `Parse-Transmute-Expr-ShiftSplit` via `SplitShiftR`.
...
* Build/test: `cmake --build /mnt/c/Dev/Cursive/cursive-bootstrap/build`; `ctest --test-dir /mnt/c/Dev/Cursive/cursive-bootstrap/build --output-on-failure` (36/36 passed; clock-skew warnings observed during build).
* Update (2026-01-06): aligned `Parse-Comptime-Unsupported` recovery with §3.3.8.6 by emitting `Unsupported-Construct` and then `SyncStmt(P)` (no brace-skipping). Updated comptime expectations in phase1/phase3 tests to include the resulting `Parse-Syntax-Err` diagnostic.

#### T-PARSE-007 Pattern parsing and helpers

**Spec:** §3.3.9 and §3.3.9.1 rules (17 + 28).
**Implement:**
...
**Completion notes (2026-01-06):**
* Implemented §3.3.9/§3.3.9.1 pattern parsing in `src/syntax/parser_patterns.cpp` with `SPEC_RULE` anchors for all Parse-Pattern* and helper rules, including tuple/field list handling and `Unsupported-Construct` trailing-comma diagnostics.
...
**Spec:** §3.3.10 and §3.3.10.1 rules; includes terminator consumption rules (`ConsumeTerminatorOpt-*`, `ConsumeTerminatorReq-*`) shown in spec. 
**Implement:**
...
**Completion notes (2026-01-06):**
* Implemented §3.3.10/§3.3.10.1 statement+block parsing in `src/syntax/parser_stmt.cpp` (ParseStmt/ParseStmtSeq/ParseBlock, terminator policies, SyncStmt rules, and all statement forms), with `SPEC_RULE` anchors for every Parse-Statement* and helper rule.
...
**Spec:** §3.3.11 rules `Attach-Doc-Line`, `Attach-Doc-Module`.
**Implement:**
...
**Spec:** §3.3.12 rules `Sync-*`; §3.3.13 diagnostic emission rule(s).
**Implement:**
...
**Spec:** §3.4.2 rules `DirOf-Root`, `DirOf-Rel`, `ReadBytes-Ok`, `ReadBytes-Err`, `ParseModule-Ok`, `ParseModule-Err-Read`, `ParseModule-Err-Load`, `Mod-Start`, `Mod-Scan`, `Mod-Scan-Err-*`, `Mod-Done`, `ParseModules-Ok`, `ParseModules-Err`.
**Implement:**
...
**Completion notes (2026-01-06):**
* Implemented frontend ParseModule/ParseModules in `src/frontend/parse_modules.cpp` with dependency injection, `ReadBytes` host-primitive behavior, and deterministic module path → directory mapping (`DirOf-*`).
...
**Spec:** §1.4 definitions ComptimeForm/UnsupportedForm and rule `Unsupported-Construct` (Cursive0 rejects all comptime forms as unsupported).
**Implement:**
...
**Spec:** §5.1.1 definitions (Γ, Σ, entity kinds, unified namespace, reserved IDs/module paths). 
**Implement:**
...
* Add `SPEC_DEF(...)` for each normative definition in §5.1.1 and `SPEC_COV` for rules that depend on them.

**Completion notes (2026-01-06):**
* Implemented scope context/data structures in `include/cursive0/sema/context.h` (Entity, Sigma, ScopeContext, scope accessors) with `SPEC_DEF` anchors for §5.1.1 definitions.
...
**Spec:** §5.1.2 rules (14) including `Intro-Ok` and duplicate/shadow constraints.
**Implement:**
...
* `ValidateModuleNames`: deterministic order Keyword → Prim → Special → Async; iterate names in sorted `IdKey` order.

**Completion notes (2026-01-06):**
* Implemented name introduction/shadow helpers and module-name validation in `src/sema/scopes_intro.cpp`; added public API in `include/cursive0/sema/scopes_intro.h` with `SPEC_RULE`/`SPEC_DEF` anchors for §5.1.2.
...
* Implemented `AliasExpand` per §5.12 (`AliasExpand-None`/`AliasExpand-Yes`) and helpers for `AliasMap`/`ModuleNames` used by `ResolveModulePath`.
* Wired `scopes_lookup.cpp` into `cursive0_sema` in `cursive-bootstrap/CMakeLists.txt`.
* Tests: added `tests/unit/sema_scopes_lookup_test.cpp` with `SPEC_COV` for §5.1.3 rules and alias expansion; registered `sema_scopes_lookup_test` in `tests/CMakeLists.txt`.
...
**Completion notes (2026-01-06):**
* Implemented visibility/accessibility helpers and rules in `include/cursive0/sema/visibility.h` and `src/sema/visibility.cpp` (`CanAccess`, `CanAccessVis`, `TopLevelVis`, `CheckModuleVisibility`) emitting `E-MOD-1207` and `E-MOD-2440` per §5.1.4.
...
* Implemented §5.1.5 top-level name collection in `include/cursive0/sema/collect_toplevel.h` and `src/sema/collect_toplevel.cpp`, including `PatNames`, `ItemOfPath`, `ResolveUsingPath`, `UsingNames`, `ItemBindings`, `CollectNames`, `NamesStart`/`NamesStep`, and `CollectNameMaps` with deterministic fixed-point iteration over modules.
...
* `include/cursive0/syntax/ast.h` (add resolved-only expr forms `Path` and `EnumLiteral` required by §5.1.6; parser must not emit them)
...
* Implemented §5.1.6 in `include/cursive0/sema/resolve_qual.h` and `src/sema/resolve_qual.cpp` (ResolveArgs/ResolveFieldInits, record/enum path resolution, QualifiedName/QualifiedApply disambiguation).
...
* Add metamorphic tests: renaming local variables shouldn’t change resolved bindings, except where shadowing is intended.
...
**Spec:** §5.2 (overall), type constructors, permission constructs, reserved type names.
**Implement:**
...
* Updated `cursive-bootstrap/docs/Cursive0.md` to define `TypeRender` and its helpers, `StateKey(⊥)=2`, and recursive `TypePaths` rules; regenerated `cursive-bootstrap/spec/rules_index.json` and `cursive-bootstrap/spec/diag_index.json`.
...
**Completion notes (2026-01-07):**
* Implemented `ConstLen` + `TypeEquiv` in `include/cursive0/sema/type_equiv.h` and `src/sema/type_equiv.cpp`, covering all 18 §5.2.1 rules with `SPEC_RULE` anchors.
...
**Completion notes (2026-01-07):**
* Implemented subtyping API in `include/cursive0/sema/subtyping.h` and `src/sema/subtyping.cpp` with rule-level `SPEC_RULE` anchors for all §5.2.2 rules, including numeric non-widening checks and TypeEquiv-based reflexivity.
...
* Added unit tests `tests/unit/sema_subtyping_test.cpp` and compile tests `tests/compile/phase3/subtyping/basic/` with `SPEC_COV` tags for all §5.2.2 rule IDs; registered in `tests/CMakeLists.txt`.
...
* Added syntax-type lowering for function types (param modes preserved), plus static value lookup for single-name statics with explicit type annotations (aligned to `StaticType`/`StaticName` in §6.7).
...
**Completion notes (2026-01-07):**
* Implemented call checking in `include/cursive0/sema/calls.h` and `src/sema/calls.cpp`, with `SPEC_RULE` anchors for all §5.2.4 rules and record-callee detection per §5.2.8.
* Added unit tests `tests/unit/sema_calls_test.cpp` with `SPEC_COV` tags for all §5.2.4 rules; registered in `tests/CMakeLists.txt`.
...
**Completion notes (2026-01-07):**
* Implemented tuple typing helpers in `include/cursive0/sema/tuples.h` and `src/sema/tuples.cpp` with rule-level `SPEC_RULE` anchors for all §5.2.5 rules, including tuple literals, tuple access (value/place), and index diagnostics.
...
**Completion notes (2026-01-07):**
* Implemented array/slice typing helpers in `include/cursive0/sema/arrays_slices.h` and `src/sema/arrays_slices.cpp` with `SPEC_RULE` anchors for all §5.2.6 rules, including array indexing, slicing, and coercion.
...
* Added unit tests `tests/unit/sema_arrays_slices_test.cpp` with `SPEC_COV` tags for all §5.2.6 rules; registered in `tests/CMakeLists.txt`.
...
**Completion notes (2026-01-07):**
* Added `include/cursive0/sema/type_infer.h` and `src/sema/type_infer.cpp` implementing all §5.2.9 rules with `SPEC_RULE` anchors, including `Syn-Expr`, `Syn-Ident`, `Syn-Unit`, `Syn-Tuple`, `Syn-Call`, `Chk-Subsumption`, `Chk-Subsumption-Modal-NonNiche`, `Chk-Null-Ptr`, `Syn-PtrNull-Err`, and `Chk-PtrNull-Err`.
...
**Spec:** §5.2.9 rules (`Syn-Call`, `Syn-Call-Err`) and §5.2.4 call diagnostics.
**Implement:**
...
**Spec:** §5.2.4 (`Call-*` rules) + §5.2.9 (`Syn-Call-Err`).
**Implement:**
...
* Implemented §5.2.11 statement typing in `src/sema/type_stmt.cpp` with `SPEC_RULE` anchors; added compound-assign non-numeric rejection that emits `Assign-Type-Err`.
...
**Completion notes (2026-01-09):**
* Implemented §5.2.12 expression typing in `src/sema/type_expr.cpp`, including `TypeCall` integration from `sema/calls`.
...
* Implemented §5.2.13 pattern typing in `src/sema/type_pattern.cpp` for wildcard, identifier, literal, typed, tuple, record, enum, modal, and range patterns; includes duplicate-binding checks, field visibility checks, and const-range validation with `Pat-StripPerm`.
* Implemented §5.2.13 match typing in `src/sema/type_match.cpp`: arm binding introduction (`IntroAll`), guard typing to `bool`, arm body typing/checking for block vs expr bodies, and exhaustiveness checks for enum/modal/union matches (irrefutable-arm fast path).
...
**Completion notes (2026-01-10):**
* Implemented §5.2.14 declaration typing in `src/sema/type_decls.cpp`, including WF checks for procedures/statics/records/enums/modals/classes/type aliases, enum variant validation (non-empty, unique variant names, unique record payload fields), and type-alias cycle detection.
...
**Spec:** §5.2.15 rules (47), including binding and move/borrow state judgments; related helper defs (e.g., access legality) appear elsewhere. 
**Implement:**
...
**Completion notes (2026-01-10):**
* Implemented §5.2.15 binding/permission state checking in `src/sema/borrow_bind.cpp` with entry points exposed in `include/cursive0/sema/borrow_bind.h`; hooked into `src/sema/type_decls.cpp` (methods/records/classes/state/transition bodies) and `cursive-bootstrap/CMakeLists.txt`.
...
**Spec:** §5.2.16 definitions: PtrState, SafePtr, PtrDiagRefs (cover via SPEC_DEF + observable pointer diagnostics tests).
**Implement:**
...
**Completion notes (2026-01-10):**
* Added safe pointer helpers + §5.2.16 `SPEC_DEF` anchors in `include/cursive0/sema/safe_ptr.h` and `src/sema/safe_ptr.cpp`; wired into `src/sema/type_expr.cpp`, `src/sema/type_infer.cpp`, and `cursive-bootstrap/CMakeLists.txt`.
...
* Added string/bytes modal widening in subtyping so `string@View`/`bytes@View` satisfy unstateful `string`/`bytes`, matching §5.8 and unblocking `RegionOptions` record literals.
...
**Spec:** §5.3.1 rules (44) incl. linearization/merge rules and conflicts. 
**Implement:**
...
**Completion notes (2026-01-11):**
* Implemented §5.3.1 linearization/merge in `src/sema/class_linearization.cpp` and effective method/field tables + dispatchability/subtyping helpers in `src/sema/classes.cpp` (with `StripPerm` spec anchor), including `EffMethods-Conflict`, `EffFields-Conflict`, and `Superclass-Undefined` handling.
...
* Refactored `src/sema/type_decls.cpp` and `src/sema/type_expr.cpp` to use the new record-method helpers, emit §5.3.2 rule anchors (`Recv-*`, `WF-Record-Method`, `T-Record-Method-Body`, `WF-Record-Methods`, `T-Record-MethodCall`, `Step-MethodCall`), and preserve existing diagnostics.
...
**Spec:** §5.4 and §5.4.1 (all bold rules; includes built-in modal type Region).
**Implement:**
...
**Completion notes (2026-01-11):**
* Implemented modal widening helpers in `src/sema/modal_widen.cpp` (+ `include/cursive0/sema/modal_widen.h`), including `PayloadState`, `NicheApplies`, `NicheCompatible`, and `WidenWarnCond` per §5.7; added `Warn-Widen-LargePayload` warning emission (`W-SYS-4010`) in `src/sema/type_expr.cpp`.
...
**Completion notes (2026-01-11):**
* Added string/bytes builtin typing in `include/cursive0/sema/string_bytes.h` and `src/sema/string_bytes.cpp` with `SPEC_DEF` anchors (`StringBytesBuiltinTable`, `StringBytesBuiltinSig`, `StringBytesJudg`) and `SPEC_RULE` anchors for all 24 §5.8 rules.
...
**Spec:** §5.11 rules (4) + §3.2.3 UniverseProtected + §5.1.7 ResolveClassPath.
**Implement:**
...
* `src/sema/typecheck_diag_map.inc` (map `Bitcopy-Clone-Missing` → `E-TYP-2623`)
**Tests:**
...
* `SPEC_COV("<RuleId>")` for all §5.11 rules.
...
**Spec:** §5.12 rules (66), including `AliasExpand-*`, `ModulePrefix-*`, `Reachable-*`, `WF-Acyclic-Eager`, and `TypeRef-*` / `ValueRef-*` rules.
**Implement:**
...
* `SPEC_COV("<RuleId>")` for all §5.12 rules.

#### T-C0-ALIGN-001 Phase 1–3 conformance fixes (multi-module + unsupported forms)

**Spec:** §3 (Phase 1), §4 (Phase 2 deferred), §5 (Phase 3), §1.1.1 (subset + permissions), §3.2.3 (reserved lexemes), §2.4 (module discovery).
**Implement:**
...
**Spec:** §5.9.4 (`BuiltinRecord`, `Fields(Context)`).
**Implement:**
...
* Spec updated to add `System` to `UniverseProtected`/`SpecialTypeNames` and to define `Implements(Context)` / `Implements(System)` as `[Bitcopy]` in §5.9.4.
...
**Spec:** §5.9.4 (`SystemInterface`, `SystemMethodSig`).
**Implement:**
...
**Spec:** §1.4 (`UnsupportedConstruct`, `UnsupportedForm`), §4 (`ComptimeForm`), §3.3.2.7 (`UnsupportedGrammarFamily`), §3.3.6.14 (parser unsupported rules).
**Implement:**
...
**Spec:** §1.4 `WF-Attr-Unsupported` (`E-UNS-0113`).
**Implement:**
...
**Spec:** §3.2.10–§3.2.12 (`LexSecure-Err`, `LexSecure-Warn`, `Tokenize-*`).
**Implement:**
...
**Spec:** §1.6.5 (`Order` identity), §3.4.2 (`ParseModule-*`).
**Implement:**
...
**Spec:** §1.1 (phase ordering), §2.1 (LoadProject), §2.4/§3.4 (module list / parse pipeline).
...
**Spec:** §2.1 (Manifest Schema + ValidateManifest), §2.2 (Assemblies), §2.4 (Module Discovery), §2.5 (Output Pipeline + Link), Program Entry Point (`MainCheck`).
**Implement:**
...
**Spec:** §6.0 rules (21) defining codegen judgments and IR model invariants.
**Implement:**
...
**Spec:** §6.1.4.* rules (union core + niche optimization subrules).
**Implement:**
...
**Spec:** §6.1.7 rules (8); §6.1.8 rules (3); §7.2 rules for modal layout dynamic semantics (`layout(M@S)`, `sizeof(M)`, `alignof(M)`, `ValueBits(M, ⟨S, v⟩)`, `ModalBits`, `NicheApplies`, `PayloadState`, `SingleFieldPayload`).
**Implement:**
...
* `src/codegen/layout_modal.cpp`
  ...
  * Implement `ValueBits(M, ⟨S, v⟩)` via `ModalBits(M, S, v)` for correct bit-level representation.
...
* `SPEC_COV("<RuleId>")` for all §6.1.7, §6.1.8, and §7.2 rules.
...
* SPEC_RULE coverage for all §6.2.4 call lowering rules.
...
* `include/cursive0/core/hash.h` (**MUST** implement FNV-1a 64-bit and Hex64 exactly per §6.3.1: FNVOffset64=14695981039346656037, FNVPrime64=1099511628211; `FNV1a64(b)`; `Hex64(x)=Hex(rev(LEBytes(x)))`).
**Tests:**
...
**Spec:** §6.3.1 rule `Mangle-Static`.
**Implement:**

* `tests/unit/codegen_mangle_test.cpp`: add a `MangleStatic` test with `SPEC_COV("Mangle-Static")`.

**Tests:**
* Unit test added above; `tools/coverage_check.py --phase 4` must report no missing `SPEC_COV` for §6.3.1.
...
* Implemented all 21 Linkage rules with SPEC_RULE anchors:
  - User items: `Linkage-UserItem`, `Linkage-UserItem-Internal` (public/internal → external, private → internal)
  - Static bindings: `Linkage-StaticBinding`, `Linkage-StaticBinding-Internal`
  - Class methods: `Linkage-ClassMethod`, `Linkage-ClassMethod-Internal` (protected → external)
...
**Spec:** §6.4 rules (177), including `Check-Range-*`, `Lower-Transmute-*`, `Lower-RawDeref-*`, and all `Lower*` judgments in §6.4. 
**Implement:**
...
* ✅ `include/cursive0/codegen/lower_expr.h` - Created with `LowerResult`, `LowerCtx`, and all expression lowering function declarations.
* ✅ `src/codegen/lower_expr_core.cpp` - Implements `LowerExpr`, `LowerLiteral`, `LowerIdentifier`, `LowerPath`, `LowerTuple`, `LowerArray`, `LowerRecord`, `LowerEnumLiteral`, `ChildrenLTR`, and helper functions `MakeIR`, `SeqIR`, `EmptyIR`.
* ✅ `src/codegen/lower_expr_calls.cpp` - Implements `LowerCall`, `LowerMethodCall`, `LowerUnaryOp`, `LowerBinaryOp` (including short-circuit `&&`/`||`), `LowerCast`, `LowerPropagate`, `LowerIf`.
* ✅ `src/codegen/lower_expr_places.cpp` - Implements `LowerPlace`, `LowerReadPlace`, `LowerWritePlace`, `LowerWritePlaceSub`, `LowerAddrOf`, `LowerMovePlace`.
* ⏸️ `src/codegen/lower_expr_checks.cpp` - Not created; check integration handled via `checks.cpp` (T-LOWER-007).
...
**Implementation Notes (2026-01-13):**
* ✅ `include/cursive0/codegen/lower_pat.h` - Created with pattern lowering function declarations.
* ✅ `src/codegen/lower_pat.cpp` - Implements `TagOf`, `LowerBindPattern` (handles `WildcardPattern`, `IdentifierPattern`, `TypedPattern`, `LiteralPattern`, `TuplePattern`, `RecordPattern`, `EnumPattern`, `ModalPattern`, `RangePattern`), `LowerBindList`, `PatternCheck`, `LowerMatchArm`, `LowerMatch`.
* All files added to `CMakeLists.txt` and build successfully.
...
**Implementation Notes (2026-01-13):**
* ✅ COMPLETED - Implemented 2026-01-14.
* ✅ `include/cursive0/codegen/globals.h` - Defines `StaticName`, `StaticBindList`, `StaticSym`, `StaticSymPath`, `EmitGlobalResult`, `EmitGlobal`, `StaticStoreIR`, `EmitStringLit`, `EmitBytesLit`, `StaticItems`, `InitSym`, `DeinitSym`, `InitFn`, `DeinitFn`, `LowerStaticInit*`, `LowerStaticDeinit*`, `InitCallIR`, `DeinitCallIR`, `EmitInitPlan`, `EmitDeinitPlan`, `EmitModuleInitFn`, `EmitModuleDeinitFn`.
* ✅ `src/codegen/globals.cpp` - Implements `StaticName`, `StaticBindList`, `StaticSym`, `EmitGlobal` (Emit-Static-Init, Emit-Static-Multi rules), `StaticStoreIR`, `EmitStringLit`, `EmitBytesLit`, `StaticItems`.
* ✅ `src/codegen/init.cpp` - Implements `InitSym`, `DeinitSym`, `InitFn`, `DeinitFn`, `LowerStaticInitItem`, `LowerStaticInitItems`, `LowerStaticInit`, `LowerStaticDeinitNames`, `LowerStaticDeinitItem`, `LowerStaticDeinitItems`, `LowerStaticDeinit`, `InitCallIR`, `DeinitCallIR`, `EmitInitPlan`, `EmitDeinitPlan`, `EmitModuleInitFn`, `EmitModuleDeinitFn`.
* All files added to `CMakeLists.txt` and build successfully.

#### T-LOWER-004A Globals and initialization correctness fixes (review)

**Spec:** §6.7 `Emit-Static-Const`, `Emit-Static-Init`, `Emit-Static-Multi`, `ConstInit`, `StaticBindTypes`, `StaticStoreIR`, `InitSym`, `DeinitSym`; §6.1 (`sizeof(T)` via layout); §6.7 `EncodeConst`.
**Implement:**
...
**Spec:** §6.9 rules (26); runtime symbols set and declarations must match pipeline rules.
**Implement:**
...
* Link-time symbol validation tests integrated with §2.5 runtime checks.
* `SPEC_COV("<RuleId>")`.

**Implementation Notes (2026-01-13):**
* ✅ COMPLETED - Implemented 2026-01-14.
* ✅ `include/cursive0/runtime/runtime_interface.h` - Defines `RegionLayoutInfo`, `RegionLayout`, `RegionSym*` (6 functions for Region methods), `BuiltinSym*` (16 FileSystem methods, 3 HeapAllocator methods), String/Bytes builtin symbols, `RuntimePanicSym`, `ContextInitSym`, and dispatch functions `RegionSym()`, `BuiltinSym()`.
* ✅ `src/codegen/runtime_calls.cpp` - Implements all `RegionSym*` functions per §6.9 rules (RegionSym-NewScoped, RegionSym-Alloc, RegionSym-ResetUnchecked, RegionSym-Freeze, RegionSym-Thaw, RegionSym-FreeUnchecked), all `BuiltinSym*` functions for FileSystem and HeapAllocator capabilities, and String/Bytes runtime symbols.
* All files added to `CMakeLists.txt` and build successfully.
...
**Implementation Notes (2026-01-13):**
* ✅ COMPLETED - Implemented 2026-01-14.
* ✅ `include/cursive0/codegen/dyn_dispatch.h` - Defines `VTableEligible`, `IsVTableEligible`, `DispatchSym`, `VTableInfo`, `VTable`, `EmitVTable`, `VSlot`, `DynPackResult`, `DynPack`, `LowerDynCall`, `DynObjectLayout`, `GetDynObjectLayout`.
* ✅ `src/codegen/dyn_dispatch.cpp` - Implements `IsVTableEligible` (filters methods with receivers), `VTableEligible`, `DispatchSym` (DispatchSym-Impl, DispatchSym-Default-None, DispatchSym-Default-Mismatch rules), `VTable` (VTable-Order rule), `EmitVTable`, `VSlot` (VSlot-Entry rule), `DynPack` (Lower-Dynamic-Form rule), `LowerDynCall` (Lower-DynCall rule), `GetDynObjectLayout`.
* All files added to `CMakeLists.txt` and build successfully.
...
**Implementation Notes (2026-01-13):**
* ✅ `include/cursive0/codegen/checks.h` - Created with `PanicReason` enum, `RangeVal`, `LowerRangeResult`, and function declarations for panic handling and checks.
* ✅ `src/codegen/checks.cpp` - Implements `PanicCode`, `PanicReasonString`, `PanicSym`, `LowerPanic`, `ClearPanic`, `PanicCheck`, `InitPanicHandle`, `CheckIndex`, `CheckRange`, `SliceBounds`, `LowerRangeExpr`, `LowerTransmute`, `LowerRawDeref`.
* ⏸️ `src/codegen/panic.cpp` - Panic functionality merged into `checks.cpp`.
* All files added to `CMakeLists.txt` and build successfully.

#### T-LOWER-008 Cleanup, drop, and unwinding

**Spec:** §6.8 rules `CleanupPlan`, `EmitDrop`, `LowerPanic`, `PanicSym`, `ClearPanic`, `PanicCheck`; definitions for `PanicReason`, `PanicCode`, `PanicSite`, `PanicReasonOf`, `WritePanicRecord`, `PanicRecordOf`, `InitPanicHandle`.
**Implement:**
...
* Wire cleanup emission into statement/block lowering (§6.5) at scope exits and before returns.
* Ensure drop order follows the spec's deterministic destruction rules (§7.4).
**Tests:**
...
* ✅ `include/cursive0/codegen/lower_expr.h` - Added `ScopeInfo` (tracks variables, defers, loop/region flags), `BindingState` (tracks type, responsibility, moved status), and scope tracking methods to `LowerCtx`: `PushScope`, `PopScope`, `CurrentScopeVars`, `VarsToLoopScope`, `VarsToFunctionRoot`, `RegisterVar`, `MarkMoved`, `MarkFieldMoved`, `GetBindingState`, `RegisterDefer`, `CurrentScopeDefers`.
* ✅ `src/codegen/lower_expr_core.cpp` - Implements all `LowerCtx` scope tracking methods.
* ✅ `src/codegen/lower_stmt.cpp` - Wired cleanup into:
...
* ✅ `src/codegen/cleanup.cpp` - Enhanced `GetBindValidity` and `GetMovedFields` to use context binding state, enhanced `ComputeCleanupPlan` to check responsibility and type from binding state.
* All files added to `CMakeLists.txt` and build successfully.
...
**Spec:** §6.0 `CG-Item-*`, §6.10 `VTable-Order`, `EmitVTable`, `Lower-DynCall`, §6.3.4 linkage rules (IR-level only).
**Implement:**
...
**Spec:** §5.1–§5.2 (resolution + typing), §6.4 `Lower-Expr-*`, `Lower-Args-*`, `Lower-RecvArg-*`.
**Implement:**
...
**Spec:** §6.12.1 definitions of LLVMHeader (TargetDataLayout, TargetTriple).
**Implement:**
...
**Spec:** §6.12.2 (opaque pointer model).
**Implement:**
...
* `SPEC_DEF("OpaquePointerModel", "§6.12.2")` plus tests validating behavior.

#### T-LLVM-003 LLVM attribute mapping for permissions and pointer state

**Spec:** §6.12.3 rules `PtrStateOf-Perm`, `LLVM-PtrAttrs-Valid`, `LLVM-PtrAttrs-Other`, `LLVM-PtrAttrs-RawPtr`, `LLVM-ArgAttrs-*` and related definitions. 
**Implement:**
...
* `SPEC_COV("<RuleId>")` for all rules in §6.12.3.

#### T-LLVM-004 UB and poison avoidance

**Spec:** §6.12.4 definitions for LLVMUBSafe constraints (no undef/poison, checked overflow/div/shifts, freeze uses, inbounds checks).
**Implement:**
...
* `SPEC_DEF("LLVMUBSafe", "§6.12.4")` and `SPEC_COV` for any labeled rules.

#### T-LLVM-005 Memory intrinsics

**Spec:** §6.12.5 rules (if any) + definitions for mem intrinsics usage.
**Implement:**
...
**Spec:** §6.12.6 rules `DeclAttrsOk`, `RuntimeDeclsOk`, `RuntimeDeclsCover` and the runtime symbol set definitions in §6.12.9 / §6.12.15 / §6.12.16 / §6.12.17.

**Implement:**

* `src/codegen/runtime_decls.cpp`

  * Centralize all runtime symbol spelling from the spec (compute symbols via the spec’s `BuiltinSym` / `PathSig` + mangling, not by hand).
...
* Prefer checking the **set of declared runtime symbols is a superset of the referenced set** (don’t require “only required declarations” unless the spec says so).
* `SPEC_COV("<RuleId>")` for each §6.12.6 rule plus any referenced runtime symbol-set rules.

#### T-LLVM-007 LLVM type mapping

**Spec:** §6.12.8 rules `LLVMTy-*` and `LLVMTy-Err` (plus §6.12.8 normative definitions like LLVMPtrTy via SPEC_DEF).
**Implement:**

* `src/codegen/llvm_types.cpp`
**Tests:**
* Unit tests for mapping each type constructor → LLVM types.
* `SPEC_COV("<RuleId>")`.

#### T-LLVM-008 LLVM IR emission pipeline

**Spec:** §6.12.9 rules (6) including `LowerIR-Module`, `LowerIR-Err`, `EmitLLVM-*`, `EmitObj-*`. 
**Implement:**
...
**Spec:** §6.12.10 rules (48), including ExpandIR/UniqueEmits invariants. 
**Implement:**
...
**Spec:** §6.12.15–§6.12.16 rules (10 total across subsections).
**Implement:**
...
**Spec:** §7.4 rules (297) and its interaction with codegen cleanup/drop.
**Implement:**
...
* Sema: capability enforcement already handled by §5.9 tasks
* Codegen: call built-in interface defined in §6.12.9 runtime symbols set 
...
**Spec:** §7.5 rule `StringLiteralVal(lit)`; relationship to `LiteralValue(lit, TypeString(@View))`.
**Implement:**

* Ensure string literal evaluation in codegen (§6.4, §6.12.14) produces values consistent with the `StringLiteralVal` judgment.
...
**Spec:** §2.5 rules `ResolveRuntimeLib-Ok`, `ResolveRuntimeLib-Err`, `Link-*`, `Out-Link-*` (plus the RuntimeLibName definition, which is `"cursive0_rt.lib"` in Cursive0).
**Implement:**
...
* **Freestanding/link-contract requirements:** the driver links with `/NODEFAULTLIB` and (per §2.5 `LinkArgsOk`) supplies only `{Objs} + cursive0_rt.lib`. Therefore `cursive0_rt.lib` MUST be self-contained: avoid depending on the MSVC CRT and the C++ standard library; disable exceptions/RTTI for runtime sources; and bundle any additional objects needed to satisfy implicit LLVM codegen/library calls (e.g., memory-operation shims and i128 div/mod helpers if LLVM lowers those ops to external symbols).

**Tests:**
* Link validation tests:

  * Deliberately remove a required symbol and ensure the spec’s `Out-Link-Runtime-Incompatible` behavior triggers (missing runtime symbol classification must happen pre-link).
...
* `SPEC_COV("ResolveRuntimeLib-Ok")`, `SPEC_COV("ResolveRuntimeLib-Err")`, plus link rules in §2.5.
...
**Spec:** bootstrap compiler notion and observable behavior equivalence concepts in §0.3; project pipeline behavior in §2.*.
**Implement:**
...
* Subcommands:
  ...
  * `build` (default)
  * `--dump` (enable the spec’s `DumpProject(P,dump=true)` command-line output behavior)
  * `--help` (usage)
...
**Spec:** §2.1 `ParseManifest-*`, §2.1 `LoadProject-*`, §2.4 `Modules-*`, §1.1 `Phase1Order`.
**Implement:**
...
**Spec:** Robustness requirement implicit in §1.2 (behavior types) and §1.3 (error recovery); no crashes or hangs on arbitrary input.
**Implement:**
...
**Spec:** §3.4.2 rules `ReadBytes-Ok`, `ReadBytes-Err`, `ParseModule-Err-Read`, `ParseModules-Err`; §3.1.8 `LoadSource-*`; §1.7 `HostPrimFail`.
**Implement:**
...
**Spec:** §1.1 `Phase1Order`, `Phase3Order`, `Phase4Order`, `WF`, `Reject-IllFormed`; §2.5 `Output-Pipeline-*`.
**Implement:**
...
**Spec:** §5.4 rules `WF-Prim`, `WF-Perm`, `WF-Tuple`, `WF-Array`, `WF-Slice`, `WF-Union`, `WF-Union-TooFew`, `WF-Func`, `WF-Path`, `WF-Dynamic`, `WF-Dynamic-Err`, `WF-String`, `WF-Bytes`, `WF-Ptr`, `WF-RawPtr`, `WF-ModalState`.
**Implement:**
...
* `src/sema/typecheck_diag_map.inc`: ensure `WF-Union-TooFew` → `E-TYP-2201` and `WF-Dynamic-Err` → `E-TYP-2509` are mapped (add if missing).
**Tests:**
...
**Spec:** §3.3.1 rules `Phase1-Complete`, `Phase1-Declarations`, `Phase1-Forward-Refs`.
**Implement:**
...
#### T-COV-002 Phase 1–3 rule coverage sweep

**Spec:** §§3–5 (all Phase 1–3 rules).
**Implement:**

* Run `tools/coverage_check.py` to enumerate missing `SPEC_COV` for §§3–5.
...
**Tests:**
* `tools/coverage_check.py` must pass for Phase 1–3 coverage.
...
* **IR tests**: compile → LLVM text; verify patterns + forbidden constructs; ensure LLVM 21.1.8 acceptance.
...
* **E2E tests**: compile → link → run; validate stdout/stderr/exit code (Windows CI baseline).
...
3. **Rule coverage**
   ...
   * For incremental bootstrapping only, allow temporary exceptions via `spec/coverage_expectations.json`—but the end state for `cursivec0` is 100%/100%.
