# Cursive0 Spec Test Suite - Ralph Loop Command

Copy and paste the command below to generate a comprehensive test suite for the Cursive0 compiler.

## Command

```
/ralph-loop:ralph-loop "Create a comprehensive, systematic compiler test suite for cursivec0 following industry-standard practices (Rust compiletest, LLVM lit). The test suite MUST validate ALL semantics and diagnostics from @cursive-bootstrap/docs/Cursive0.md.
...
1. Restart Claude Code to load the ralph-loop plugin (if not already loaded)
2. Copy the entire command above (from `/ralph-loop` to the closing quote)
3. Paste into Claude Code and press Enter
4. The loop will run autonomously until completion or max iterations
