...
public:
  ...
  llvm::Type* GetLLVMType(sema::TypeRef type);
  ...
  // T-LLVM-011: Call ABI
  ABICallResult ComputeCallABI(const std::vector<IRParam>& params, sema::TypeRef ret_type);
...
private:
  ...
  // Type cache
  std::unordered_map<sema::TypeRef, llvm::Type*> type_cache_;
...
public:
  ...
  void AddPtrAttributes(llvm::Function* func, unsigned arg_idx, sema::TypeRef type);
