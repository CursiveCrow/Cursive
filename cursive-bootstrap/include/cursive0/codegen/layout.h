...
struct UnionLayout {
  ...
  std::uint64_t payload_size = 0;
  std::uint64_t payload_align = 1;
  std::vector<cursive0::sema::TypeRef> member_list;
};
...
std::optional<RecordLayout> RecordLayoutOf(
    const cursive0::sema::ScopeContext& ctx,
    const std::vector<cursive0::sema::TypeRef>& fields);
...
struct UnitVal {};
struct PtrVal { cursive0::sema::PtrState state; std::uint64_t addr = 0; };
struct RawPtrVal { cursive0::sema::RawPtrQual qual; std::uint64_t addr = 0; };
