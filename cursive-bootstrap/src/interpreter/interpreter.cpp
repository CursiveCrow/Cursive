...
struct ScopeContextRestorer {
  explicit ScopeContextRestorer(sema::ScopeContext& ctx)
      : ctx_(&ctx),
        snapshot_(ctx) {}
 ...
 private:
  sema::ScopeContext* ctx_ = nullptr;
  sema::ScopeContext snapshot_;
};
...
InterpreterResult InterpretExpr(const syntax::Expr& expr,
  ...
  auto name_maps = sema::CollectNameMaps(scope);
...
InterpreterResult InterpretProject(const project::Project& project,
  ...
  if (!main_result.module_path.empty()) {
    ...
    if (it != name_maps.name_maps.end()) {
      scope.scopes = {sema::Scope{}, it->second, sema::UniverseBindings()};
    }
  }
