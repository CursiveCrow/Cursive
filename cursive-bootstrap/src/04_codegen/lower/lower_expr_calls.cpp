...
#include "cursive0/04_codegen/dyn_dispatch.h"
...
LowerResult LowerMethodCall(const syntax::MethodCallExpr& expr, LowerCtx& ctx) {
  ...
  if (ctx.sigma && ctx.expr_type) {
    analysis::ScopeContext sym_scope;
...
LowerResult LowerCast(const syntax::Expr& expr,
  ...
  if (target_type) {
    ...
    if (stripped_target && std::holds_alternative<analysis::TypeDynamic>(stripped_target->node)) {
      SPEC_RULE("Eval-Dynamic-Form");
      SPEC_RULE("Eval-Dynamic-Form-Ctrl");

      const auto* dyn = std::get_if<analysis::TypeDynamic>(&stripped_target->node);
      ...
      analysis::TypeRef stripped_expr = expr_type ? analysis::StripPerm(expr_type) : expr_type;
      const auto class_it = ctx.sigma->classes.find(analysis::PathKeyOf(dyn->path));
...
LowerResult LowerPropagateExpr(const syntax::PropagateExpr& expr,
  ...
  std::optional<analysis::TypeRef> success_type;
