...
bool IsNicheType(const cursive0::analysis::ScopeContext& ctx,
                 const cursive0::analysis::TypeRef& type) {
  (void)ctx;
  if (!type) {
    return false;
  ...
  if (const auto* ptr = std::get_if<cursive0::analysis::TypePtr>(&type->node)) {
    return ptr->state == cursive0::analysis::PtrState::Valid;
  }
  return false;
}
...
bool IsNicheBits(const cursive0::analysis::ScopeContext& ctx,
                 const cursive0::analysis::TypeRef& type,
                 const std::vector<std::uint8_t>& bits) {
...
const Value* FindFieldValue(const RecordVal& val, std::string_view name) {
  for (const auto& entry : val.fields) {
    if (cursive0::analysis::IdEq(entry.first, name)) {
      return &entry.second;
    }
  }
  return nullptr;
...
const cursive0::syntax::StateBlock* FindState(
  ...
  for (const auto& state : decl.states) {
    if (cursive0::analysis::IdEq(state.name, name)) {
      return &state;
    }
  }
  return nullptr;
...
std::optional<std::pair<std::string, cursive0::analysis::TypeRef>>
SingleFieldPayloadType(const cursive0::analysis::ScopeContext& ctx,
...
std::optional<std::vector<std::pair<std::string, cursive0::analysis::TypeRef>>> CollectStateFields(
    const cursive0::analysis::ScopeContext& ctx,
    const cursive0::syntax::StateBlock& state) {
  std::vector<std::pair<std::string, cursive0::analysis::TypeRef>> fields;
  for (const auto& member : state.members) {
    if (const auto* field =
            std::get_if<cursive0::syntax::StateFieldDecl>(&member)) {
      const auto lowered = LowerTypeForLayout(ctx, field->type);
...
std::optional<std::vector<std::uint8_t>> ValueBitsForRecord(
    const cursive0::analysis::ScopeContext& ctx,
    const std::vector<std::pair<std::string, cursive0::analysis::TypeRef>>& fields,
    const RecordVal& val) {
  std::vector<std::vector<std::uint8_t>> bits;
  std::vector<cursive0::analysis::TypeRef> types;
  ...
  for (const auto& field : fields) {
    const auto it = std::find_if(val.fields.begin(), val.fields.end(),
                                [&](const auto& entry) {
                                  return cursive0::analysis::IdEq(entry.first,
                                                             field.first);
                                });
...
std::optional<std::vector<std::uint8_t>> ModalTaggedBits(
  ...
  for (std::size_t i = 0; i < decl.states.size(); ++i) {
    ...
    if (cursive0::analysis::IdEq(st.name, state)) {
      found = true;
      state_index = i;
...
std::optional<std::vector<std::uint8_t>> ModalNicheBits(
  ...
  if (cursive0::analysis::IdEq(*payload_state, state)) {
...
bool ValidStructBits(const cursive0::analysis::ScopeContext& ctx,
                     const std::vector<cursive0::analysis::TypeRef>& types,
                     const std::vector<std::uint64_t>& offsets,
                     std::uint64_t size,
                     const std::vector<std::uint8_t>& bits) {
...
bool ValidPaddedStructBits(const cursive0::analysis::ScopeContext& ctx,
                           const std::vector<cursive0::analysis::TypeRef>& types,
                           const std::vector<std::uint64_t>& offsets,
...
bool ValidTaggedPayloadBits(const cursive0::analysis::ScopeContext& ctx,
                            const cursive0::analysis::TypeRef& type,
                            const std::vector<std::uint8_t>& payload_bits) {
  const auto size = SizeOf(ctx, type);
  if (!size.has_value()) {
...
bool ValidEnumBits(const cursive0::analysis::ScopeContext& ctx,
                   const cursive0::syntax::EnumDecl& decl,
                   const std::vector<std::uint8_t>& bits) {
...
bool ValidModalTaggedBits(const cursive0::analysis::ScopeContext& ctx,
                          const cursive0::syntax::ModalDecl& decl,
                          const std::vector<std::uint8_t>& bits) {
  const auto layout = ModalLayoutOf(ctx, decl);
  if (!layout.has_value() || layout->niche || !layout->disc_type.has_value()) {
...
bool ValidModalNicheBits(const cursive0::analysis::ScopeContext& ctx,
                         const cursive0::syntax::ModalDecl& decl,
                         const std::vector<std::uint8_t>& bits) {
  const auto layout = ModalLayoutOf(ctx, decl);
...
bool ValidModalBits(const cursive0::analysis::ScopeContext& ctx,
                    const cursive0::syntax::ModalDecl& decl,
                    const std::vector<std::uint8_t>& bits) {
  const auto layout = ModalLayoutOf(ctx, decl);
  if (!layout.has_value()) {
...
bool ValidUnionBits(const cursive0::analysis::ScopeContext& ctx,
...
std::optional<std::vector<std::uint8_t>> EncodeConst(
  ...
  const auto* prim = std::get_if<cursive0::analysis::TypePrim>(&type->node);
  if (!prim) {
...
bool ValidValue(const cursive0::analysis::ScopeContext& ctx,
  ...
  if (const auto* ptr = std::get_if<cursive0::analysis::TypePtr>(&type->node)) {
    ...
    if (ptr->state == cursive0::analysis::PtrState::Null) {
      return bits == zero;
    }
  ...
  if (std::holds_alternative<cursive0::analysis::TypeRange>(type->node)) {
    ...
    std::vector<cursive0::analysis::TypeRef> types;
    types.push_back(cursive0::analysis::MakeTypePrim("u8"));
    types.push_back(cursive0::analysis::MakeTypePrim("usize"));
    types.push_back(cursive0::analysis::MakeTypePrim("usize"));
  ...
  if (std::holds_alternative<cursive0::analysis::TypeString>(type->node)) {
    const auto size = SizeOf(ctx, type);
    return size.has_value() && bits.size() == *size;
  }
  if (std::holds_alternative<cursive0::analysis::TypeBytes>(type->node)) {
    const auto size = SizeOf(ctx, type);
    return size.has_value() && bits.size() == *size;
  }
  return false;
...
std::optional<std::vector<std::uint8_t>> ValueBits(
  ...
  if (const auto* ptr = std::get_if<cursive0::analysis::TypePtr>(&type->node)) {
    if (const auto* v = std::get_if<PtrVal>(&value.node)) {
      ...
      if (v->state == cursive0::analysis::PtrState::Valid && v->addr == 0) {
        return std::nullopt;
      }
      if (v->state == cursive0::analysis::PtrState::Null && v->addr != 0) {
        return std::nullopt;
      }
  ...
  if (const auto* raw = std::get_if<cursive0::analysis::TypeRawPtr>(&type->node)) {
  ...
  if (std::holds_alternative<cursive0::analysis::TypeSlice>(type->node)) {
    ...
    if (v->ptr.qual != cursive0::analysis::RawPtrQual::Imm) {
      return std::nullopt;
    }
  ...
  if (std::holds_alternative<cursive0::analysis::TypeRange>(type->node)) {
    ...
    std::vector<std::pair<std::string, cursive0::analysis::TypeRef>> fields;
    fields.emplace_back("kind", cursive0::analysis::MakeTypePrim("u8"));
    fields.emplace_back("lo", cursive0::analysis::MakeTypePrim("usize"));
    fields.emplace_back("hi", cursive0::analysis::MakeTypePrim("usize"));
    RecordVal rv;
  ...
  if (const auto* path =
    ...
    if (const auto* enum_decl =
      ...
      for (std::size_t i = 0; i < enum_decl->variants.size(); ++i) {
        if (cursive0::analysis::IdEq(enum_decl->variants[i].name, v->variant)) {
          index = i;
          found = true;
          break;
        }
      ...
      const auto discs = cursive0::analysis::EnumDiscriminants(*enum_decl);
    ...
    if (const auto* modal =
      ...
      if (cursive0::analysis::NicheApplies(ctx, *modal)) {
  ...
  if (const auto* uni = std::get_if<cursive0::analysis::TypeUnion>(&type->node)) {
    ...
    if (layout->niche) {
      ...
      for (std::size_t i = 0; i < members.size(); ++i) {
        if (cursive0::analysis::TypeEquiv(v->member, members[i]).equiv) {
          member_index = i;
          break;
        }
      }
    ...
    for (std::size_t i = 0; i < layout->member_list.size(); ++i) {
      if (cursive0::analysis::TypeEquiv(v->member, layout->member_list[i]).equiv) {
        index = i;
        found = true;
        break;
      }
