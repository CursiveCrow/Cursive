...
std::optional<sema::ParamMode> LowerParamMode(std::optional<syntax::ParamMode> mode) {
  if (!mode.has_value()) {
    return std::nullopt;
  }
  switch (*mode) {
    case syntax::ParamMode::Move:
      return sema::ParamMode::Move;
  }
  return std::nullopt;
}

sema::TypeRef TypePrim(std::string_view name) {
  return sema::MakeTypePrim(std::string(name));
}

sema::TypeRef TypePath(std::initializer_list<std::string> names) {
  sema::TypePath path;
...
sema::TypeRef TypeModalState(std::initializer_list<std::string> path,
                             std::string_view state) {
  sema::TypePath out;
  for (const auto& name : path) {
    out.push_back(name);
  }
  return sema::MakeTypeModalState(std::move(out), std::string(state));
}

sema::TypeRef TypeUnion(std::vector<sema::TypeRef> members) {
...
void LLVMEmitter::DeclareRuntime() {
  ...
  const sema::ScopeContext scope = BuildScope(current_ctx_);
  ...
  auto declare_fn = [&](const std::string& name,
    ...
    if (abi.has_sret) {
      auto sret_ptr = sema::MakeTypePerm(
          sema::Permission::Unique,
          sema::MakeTypePtr(ret, sema::PtrState::Valid));
    ...
    for (std::size_t i = 0; i < params.size(); ++i) {
      ...
      if (abi.param_kinds[i] == PassKind::ByRef) {
        auto byref_ptr = sema::MakeTypePerm(
            sema::Permission::Const,
            sema::MakeTypePtr(params[i].type, sema::PtrState::Valid));
  ...
  // Panic
  {
    ...
    params.push_back(MakeParam("code", sema::ParamMode::Move, TypePrim("u32")));
  ...
  {
    std::vector<IRParam> params;
    params.push_back(MakeParam("value", sema::ParamMode::Move,
                               sema::MakeTypeString(sema::StringState::Managed)));
    declare_fn(BuiltinSymStringDropManaged(), params, TypePrim("()"), false);
  }
  {
    std::vector<IRParam> params;
    params.push_back(MakeParam("value", sema::ParamMode::Move,
                               sema::MakeTypeBytes(sema::BytesState::Managed)));
  ...
  }
  {
    ...
    auto self_ty = sema::MakeTypePerm(sema::Permission::Unique, region_active);
  ...
  }
  {
    ...
    auto self_ty = sema::MakeTypePerm(sema::Permission::Unique, region_frozen);
    params.push_back(MakeParam("self", std::nullopt, self_ty));
  ...
  }
  {
    ...
    auto self_ty = sema::MakeTypePerm(sema::Permission::Unique, union_ty);
    params.push_back(MakeParam("self", std::nullopt, self_ty));
  ...
  // String builtins
  {
    ...
    for (const auto& name : names) {
      auto func_type = sema::LookupStringBytesBuiltinType(path, name);
      if (!func_type || !std::holds_alternative<sema::TypeFunc>((*func_type)->node)) {
        if (current_ctx_) {
          current_ctx_->ReportCodegenFailure();
        }
        continue;
      }
      const auto& func = std::get<sema::TypeFunc>((*func_type)->node);
      std::vector<IRParam> params;
  ...
  // FileSystem builtins
  {
    ...
    for (const auto& name : names) {
      ...
      std::vector<IRParam> params;
      auto self_ty = sema::MakeTypePerm(sig->recv_perm,
                                        sema::MakeTypeDynamic({"FileSystem"}));
      params.push_back(MakeParam("self", std::nullopt, self_ty));
      ...
      for (const auto& param : sig->params) {
        ...
        params.push_back(MakeParam(param.name, param.mode.has_value()
                                                ? std::optional<sema::ParamMode>(sema::ParamMode::Move)
                                                : std::nullopt,
                                   *lowered));
      }
  ...
  // HeapAllocator builtins
  {
    ...
    for (const auto& name : names) {
      ...
      std::vector<IRParam> params;
      auto self_ty = sema::MakeTypePerm(sig->recv_perm,
                                        sema::MakeTypeDynamic({"HeapAllocator"}));
      params.push_back(MakeParam("self", std::nullopt, self_ty));
  ...
  // File/DirIter modal methods and transitions
  {
      ...
      for (const auto& state : decl.states) {
        for (const auto& member : state.members) {
          ...
          if (const auto* trans = std::get_if<syntax::TransitionDecl>(&member)) {
            std::vector<IRParam> params;
            auto self_ty = TypeModalState({decl.name}, state.name);
            params.push_back(MakeParam("self", sema::ParamMode::Move, self_ty));
            ...
            sema::TypeRef ret = TypeModalState({decl.name}, trans->target_state);
    ...
    declare_modal(sema::BuildFileModalDecl());
    declare_modal(sema::BuildDirIterModalDecl());
  }
}
