...
IRParam MakeParam(std::string name, std::optional<sema::ParamMode> mode, sema::TypeRef type) {
  IRParam param;
  param.mode = mode;
...
llvm::Module* LLVMEmitter::EmitModule(const IRDecls& decls, LowerCtx& ctx) {
  ...
  for (const auto& entry : ctx.drop_glue_types) {
    ...
    const sema::TypeRef& type = entry.second;
    bool exists = false;
    ...
    ProcIR proc;
    proc.symbol = sym;
    proc.params.push_back(MakeParam("data", sema::ParamMode::Move,
                                    sema::MakeTypeRawPtr(sema::RawPtrQual::Imm,
                                                         sema::MakeTypePrim("()"))));
    proc.params.push_back(MakeParam(std::string(kPanicOutName), sema::ParamMode::Move,
                                    PanicOutType()));
