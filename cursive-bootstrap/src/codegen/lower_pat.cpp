...
static sema::TypeRef LowerSyntaxType(const std::shared_ptr<syntax::Type>& type,
                                    LowerCtx& ctx) {
  if (!type || !ctx.sigma) {
    return nullptr;
  }
...
static const syntax::ModalDecl* LookupModalDecl(const sema::TypePath& path,
                                                const LowerCtx& ctx) {
  if (!ctx.sigma) {
    return nullptr;
  }
...
static sema::TypeRef RecordFieldType(const syntax::RecordDecl& decl,
                                     std::string_view name,
                                     LowerCtx& ctx) {
  for (const auto& member : decl.members) {
    if (const auto* field = std::get_if<syntax::FieldDecl>(&member)) {
      if (sema::IdEq(field->name, name)) {
...
static const syntax::VariantDecl* FindVariant(const syntax::EnumDecl& decl,
  ...
  for (const auto& variant : decl.variants) {
    if (sema::IdEq(variant.name, name)) {
      return &variant;
    }
  }
  return nullptr;
...
static sema::TypeRef EnumPayloadFieldType(const syntax::VariantDecl& variant,
                                          std::string_view name,
                                          LowerCtx& ctx) {
...
static sema::TypeRef EnumPayloadTupleType(const syntax::VariantDecl& variant,
                                          std::size_t index,
                                          LowerCtx& ctx) {
  if (!variant.payload_opt.has_value()) {
    return nullptr;
...
static sema::TypeRef ModalFieldType(const syntax::ModalDecl& decl,
  ...
  for (const auto& member : state->members) {
    if (const auto* field = std::get_if<syntax::StateFieldDecl>(&member)) {
      if (sema::IdEq(field->name, field_name)) {
        return LowerSyntaxType(field->type, ctx);
      }
    }
  }
        ...
        std::visit(
            [&](const auto& node) {
              ...
              if constexpr (std::is_same_v<T, syntax::WildcardPattern>) {
                ...
                if (hint && std::holds_alternative<sema::TypeTuple>(hint->node)) {
                  tuple_type = &std::get<sema::TypeTuple>(hint->node);
                }
                ...
                if (hint && std::holds_alternative<sema::TypeModalState>(hint->node)) {
                  modal_path = std::get<sema::TypeModalState>(hint->node).path;
                } else if (hint && std::holds_alternative<sema::TypePathType>(hint->node)) {
                  modal_path = std::get<sema::TypePathType>(hint->node).path;
                }
...
LowerResult LowerMatchArm(const syntax::MatchArm& arm,
                          ...
                          const sema::TypeRef& scrutinee_type,
                          LowerCtx& ctx) {
