...
bool IsValidPtrType(const sema::TypeRef& type) {
  const auto stripped = StripPerm(type);
  if (!stripped) {
    return false;
  }
  if (const auto* ptr = std::get_if<sema::TypePtr>(&stripped->node)) {
    return ptr->state.has_value() && *ptr->state == sema::PtrState::Valid;
  }
  return false;
}
...
ABICallResult LLVMEmitter::ComputeCallABI(const std::vector<IRParam>& params,
  ...
  std::vector<std::pair<std::optional<sema::ParamMode>, sema::TypeRef>> abi_params;
    ...
    if (current_ctx_) {
      if (std::getenv("CURSIVE0_DEBUG_OBJ")) {
        std::cerr << "[cursivec0] ABI call failed: ret="
                  << sema::TypeToString(ret_type) << " params=[";
        for (std::size_t i = 0; i < params.size(); ++i) {
          ...
          if (param.mode.has_value() &&
              *param.mode == sema::ParamMode::Move) {
            std::cerr << "move ";
          }
          std::cerr << sema::TypeToString(param.type);
        }
        std::cerr << "]\n";
      }
