...
static sema::TypeRef LoopPatternType(const sema::TypeRef& iter_type) {
  if (!iter_type) {
    return nullptr;
  }
  if (std::holds_alternative<sema::TypeArray>(iter_type->node)) {
    return std::get<sema::TypeArray>(iter_type->node).element;
  }
  if (std::holds_alternative<sema::TypeSlice>(iter_type->node)) {
    return std::get<sema::TypeSlice>(iter_type->node).element;
  }
  return nullptr;
}
...
IRPtr LowerStmt(const syntax::Stmt& stmt, LowerCtx& ctx) {
  ...
  IRPtr ir = std::visit(
      [&ctx, &temps, &temps_handled](const auto& node) -> IRPtr {
        ...
        auto block_result_type = [&ctx](const std::shared_ptr<syntax::Block>& block) -> sema::TypeRef {
          if (!block) {
            return nullptr;
          }
