...
void AppendStructElems(std::vector<llvm::Type*>& elems,
                       const std::vector<sema::TypeRef>& fields,
...
std::optional<sema::TypeRef> ModalNichePayloadType(
  ...
  for (const auto& state : decl.states) {
    if (!sema::IdEq(state.name, *payload_state)) {
      continue;
    }
...
std::vector<sema::TypeRef> RecordFieldTypes(const sema::ScopeContext& scope,
...
std::vector<sema::TypeRef> ModalStateFields(const sema::ScopeContext& scope,
                                            const syntax::ModalDecl& decl,
                                            std::string_view state_name) {
  for (const auto& state : decl.states) {
    if (!sema::IdEq(state.name, state_name)) {
      continue;
    }
    std::vector<sema::TypeRef> fields;
    for (const auto& member : state.members) {
...
std::optional<std::uint64_t> MaxModalPayloadAlign(const sema::ScopeContext& scope,
                                                  const syntax::ModalDecl& decl) {
...
llvm::Type* LLVMEmitter::GetLLVMType(sema::TypeRef type) {
  if (!type) {
  ...
  if (const auto* prim = std::get_if<sema::TypePrim>(&type->node)) {
  ...
  } else if (const auto* perm = std::get_if<sema::TypePerm>(&type->node)) {
    SPEC_RULE("LLVMTy-Perm");
    ll_ty = GetLLVMType(perm->base);
  } else if (std::holds_alternative<sema::TypePtr>(type->node)) {
    SPEC_RULE("LLVMTy-Ptr");
    ll_ty = GetOpaquePtr();
  } else if (std::holds_alternative<sema::TypeRawPtr>(type->node)) {
    SPEC_RULE("LLVMTy-RawPtr");
    ll_ty = GetOpaquePtr();
  } else if (std::holds_alternative<sema::TypeFunc>(type->node)) {
    SPEC_RULE("LLVMTy-Func");
    ll_ty = GetOpaquePtr();
  } else if (const auto* tuple = std::get_if<sema::TypeTuple>(&type->node)) {
  ...
  } else if (const auto* arr = std::get_if<sema::TypeArray>(&type->node)) {
    SPEC_RULE("LLVMTy-Array");
    llvm::Type* elem_ty = GetLLVMType(arr->element);
    ll_ty = llvm::ArrayType::get(elem_ty, arr->length);
  } else if (const auto* slice = std::get_if<sema::TypeSlice>(&type->node)) {
    SPEC_RULE("LLVMTy-Slice");
    llvm::Type* ptr_ty = GetOpaquePtr();
    llvm::Type* len_ty = GetLLVMType(sema::MakeTypePrim("usize"));
    ll_ty = llvm::StructType::get(context_, {ptr_ty, len_ty});
  } else if (const auto* str = std::get_if<sema::TypeString>(&type->node)) {
    if (str->state.has_value() && *str->state == sema::StringState::View) {
      SPEC_RULE("LLVMTy-StringView");
      llvm::Type* ptr_ty = GetOpaquePtr();
      llvm::Type* len_ty = GetLLVMType(sema::MakeTypePrim("usize"));
      ll_ty = llvm::StructType::get(context_, {ptr_ty, len_ty});
    } else if (str->state.has_value() && *str->state == sema::StringState::Managed) {
  ...
  } else if (const auto* bytes = std::get_if<sema::TypeBytes>(&type->node)) {
    if (bytes->state.has_value() && *bytes->state == sema::BytesState::View) {
      SPEC_RULE("LLVMTy-BytesView");
      llvm::Type* ptr_ty = GetOpaquePtr();
      llvm::Type* len_ty = GetLLVMType(sema::MakeTypePrim("usize"));
      ll_ty = llvm::StructType::get(context_, {ptr_ty, len_ty});
    } else if (bytes->state.has_value() && *bytes->state == sema::BytesState::Managed) {
  ...
  } else if (std::holds_alternative<sema::TypeDynamic>(type->node)) {
    ...
    ll_ty = llvm::StructType::get(context_, elems);
  } else if (std::holds_alternative<sema::TypeModalState>(type->node)) {
    ...
    if (!scope_opt.has_value()) {
      ...
      const auto* modal = std::get_if<sema::TypeModalState>(&type->node);
      syntax::Path syntax_path;
      syntax_path.reserve(modal->path.size());
      for (const auto& comp : modal->path) {
        syntax_path.push_back(comp);
      }
      const auto it = scope_opt->sigma.types.find(sema::PathKeyOf(syntax_path));
  ...
  } else if (std::holds_alternative<sema::TypeRange>(type->node)) {
  ...
  } else if (const auto* uni = std::get_if<sema::TypeUnion>(&type->node)) {
    if (!scope_opt.has_value()) {
  ...
  } else if (const auto* path = std::get_if<sema::TypePathType>(&type->node)) {
    if (!scope_opt.has_value()) {
      SPEC_RULE("LLVMTy-Err");
      ll_ty = GetOpaquePtr();
    } else {
