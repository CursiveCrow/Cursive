...
static std::optional<TypeRef> IterElementType(const TypeRef& iter_type) {
  const auto stripped = StripPermOnce(iter_type);
  if (!stripped) {
    return std::nullopt;
  }
      ...
      [&](const auto& node) -> PatternTypeResult {
        ...
        if constexpr (std::is_same_v<T, syntax::WildcardPattern>) {
          ...
          for (const auto& field : node.fields) {
            ...
            const auto lowered = LocalLowerType(ctx, decl->type);
            if (!lowered.ok) {
...
PatternTypeResult TypePattern(const ScopeContext& ctx,
  ...
  const auto expected_base = StripPermOnce(expected);
  if (!expected_base) {
    return result;
  }
...
ExprTypeResult TypeLoopIterExpr(const ScopeContext& ctx,
  ...
  if (const auto async_iter = AsyncSigOf(ctx, iter_type.type)) {
    ...
    if (expr.type_opt) {
      const auto lowered = LocalLowerType(ctx, expr.type_opt);
      if (!lowered.ok) {
  ...
  return std::visit(
      [&](const auto& node) -> StmtTypeResult {
        ...
        if constexpr (std::is_same_v<T, syntax::LetStmt>) {
          ...
          const auto sub = Subtyping(ctx, rhs_type.type, StripPermOnce(place_type.type));
          if (!sub.ok) {
