# Cursive Language Feature Analysis

For a user-facing handbook built from this analysis, see [CursiveUserGuide.md](./CursiveUserGuide.md).

This document summarizes the primary language surface exposed by [CursiveSpecification.md](./CursiveSpecification.md), with emphasis on the features that most clearly define Cursive as a language rather than as a conventional systems-language syntax. It is written for language evaluation and AI/LLM code authoring. The feature ranking below is based on how much each feature shapes ordinary program structure, correctness, and effect boundaries.

Use cases in this document are inferred from the normative semantics in the specification; they are not copied from tutorial or marketing text.

## Primary Surface Summary

The primary Cursive surface is a systems language organized around explicit authority, explicit state, and explicit concurrency. Its most distinctive layers are: capability-gated effects with no ambient authority; an ownership model that separates responsibility transfer from aliasing permission; modal state-machine types; region/frame allocation; a first-class key system for synchronized shared access; structured CPU/GPU parallelism; async expressed as resumable modal state; static-by-default contracts and refinement types; and compile-time execution with reflection and emission. Ordinary data types, generics, classes, and methods are present, but they serve these stronger control surfaces rather than defining the language on their own.

## Primary and Unique Features
...
If Cursive is reduced to its core identity, the language surface is best understood as:

1. explicit authority,
2. explicit ownership and aliasing,
3. explicit state machines,
4. explicit scoped memory,
5. explicit synchronized sharing,
6. explicit structured concurrency, and
7. explicit proof obligations.

That combination is what most strongly distinguishes Cursive from a conventional "Rust-like" or "C-like" systems language with added libraries.
