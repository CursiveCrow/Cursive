...
// Find the [[log]] attribute item in an attribute list (or nullptr).
const ast::AttributeItem* FindLogAttr(const ast::AttributeList& attrs);

// Find a named argument token in an attribute item (e.g., label, expected).
const ast::Token* FindNamedTokenArg(const ast::AttributeItem& attr,
                                    std::string_view key);

// Sanitize text for trace payloads (replace \t, \n, \r with spaces).
// Delimiters used by field parsing (';') are normalized to ':'.
std::string SanitizePayloadText(std::string text);

// Strip surrounding quotes from a literal lexeme.
std::string StripQuotedLiteral(std::string text);

// Extract the label string from a [[log]] attribute, if present.
std::optional<std::string> LogLabelFor(const ast::AttributeList& attrs);

// Extract the expected lexeme from a [[log]] attribute, if present.
std::optional<std::string> LogExpectedFor(const ast::AttributeList& attrs);

// Build shared [[log]] payload prefix ending in `;actual=`.
// Runtime span columns now carry source location; payload focuses on label and
// comparison fields.
std::string BuildLogPayloadPrefix(
    const ast::AttributeList& attrs,
    const core::Span& span,
    std::string_view target_kind,
    std::optional<std::string_view> cmp_result = std::nullopt);

// Build common procedure-level [[log]] payload fields before `;actual=`.
std::string BuildProcedureLogPayloadBase(
    std::string_view phase,
    std::string_view procedure_name,
    const std::optional<std::string>& label,
    const std::optional<std::string>& expected,
    std::optional<lexer::TokenKind> expected_token_kind = std::nullopt,
    bool include_expected = true);

// Emit runtime trace record with a preformatted payload.
IRPtr EmitRuntimeTraceRecord(
    std::string_view rule_id,
    std::string_view payload,
    const std::optional<core::Span>& span = std::nullopt);

// Emit runtime trace record with a concrete runtime `actual` value suffix.
// `payload_prefix` MUST include `actual=` at the end.
IRPtr EmitRuntimeTraceWithActual(std::string_view rule_id,
                                 ...
                                 const std::optional<core::Span>& span = std::nullopt,
                                 const LowerCtx* lower_ctx = nullptr);

// Emit runtime trace record where `actual` is unit `()`.
// `payload_prefix` MUST include `actual=` at the end.
IRPtr EmitRuntimeTraceUnitActual(std::string_view rule_id,
                                 std::string payload_prefix,
                                 const std::optional<core::Span>& span = std::nullopt);

// Emit runtime conformance record for [[log]] attributes on expressions/places.
// When the attribute has an `expected` argument and the type supports equality
// comparison, emits a pass/fail cmp field alongside the actual value.
...
// Emit procedure-level log trace with optional pass/fail comparison.
// `payload_base` is the prefix ending BEFORE `;actual=` (the function appends
// `;cmp=pass;actual=` or `;cmp=fail;actual=` or just `;actual=` as needed).
...
// Unwrap an AttributedExpr to get the inner expression
