...
class LspServer {
 ...
 private:
  ...
  void AnalyzeProject(const std::filesystem::path& project_root);
  void AnalyzeProjectForPath(const std::filesystem::path& path);
  ...
  ProjectSnapshot* SnapshotForPath(const std::filesystem::path& path);
  const ProjectSnapshot* SnapshotForPath(const std::filesystem::path& path) const;
  ...
  std::unordered_map<std::string, ProjectSnapshot> projects_by_root_;
