...
struct AssemblyImportContext {
  ...
  std::map<std::string, ast::Path> exported_items;  // name -> path
...
class ImportResolver {
 public:
  ...
  bool IsAccessible(const ast::Path& target,
...
struct CircularImportResult {
  ...
  std::vector<ast::Path> cycle_path;
};
...
struct ExtendedUsingClause {
  ast::Path path;
  std::vector<std::shared_ptr<ast::Type>> generic_args;
  std::optional<ast::Identifier> alias;
};
