...
ParseElemResult<Stmt> ParseStmt(Parser parser) {
  ...
  const auto starts_non_expr_stmt = [&](const Token* tok) {
    if (!tok) {
      return false;
    }
  ...
  const auto has_implicit_line_terminator_after_log_attr = [&]() {
    if (!attrs_all_log || !attrs.elem.has_value()) {
      return false;
    }
    const Token* next = Tok(parser);
    if (!next) {
      return false;
    }
    // Newline filtering can remove the physical newline after `]]`. Recover the
    // standalone-statement form when the next token starts on a later source
    // line and cannot begin an expression.
    const bool crossed_line =
        attr_list.back().span.end_line < next->span.start_line;
    return crossed_line &&
           (!IsExprStartToken(*next) || starts_non_expr_stmt(next));
  }();

  // Standalone [[log(...)]] statement form: `[[log(...)]]` followed directly
  // by a statement terminator.
  if (attrs_all_log &&
      ((Tok(parser) && Tok(parser)->kind == TokenKind::Newline) ||
       IsPunc(parser, ";") ||
       has_implicit_line_terminator_after_log_attr)) {
    LogStmt log_stmt;
    log_stmt.attrs = *attrs.elem;
    log_stmt.span = log_stmt.attrs.front().span;
    SPEC_RULE_AT("Parse-LogStmt", log_stmt.span);
    Parser next = parser;
    ConsumeTerminatorOpt(next, TerminatorPolicy::Required);
    return {next, std::move(log_stmt)};
  }
