// =============================================================================
// shadow_var_stmt.cpp - Shadow var statement typing
// =============================================================================
//
// SPEC REFERENCE: CursiveSpecification.md
//   Section 5.2.11: Statement Typing (lines 9429-9447)
...
static inline void SpecDefsShadowVarStmt() {
  SPEC_DEF("T-ShadowVarStmt-Ann", "5.2.11");
  SPEC_DEF("T-ShadowVarStmt-Ann-Mismatch", "5.2.11");
  SPEC_DEF("T-ShadowVarStmt-Infer", "5.2.11");
  SPEC_DEF("T-ShadowVarStmt-Infer-Err", "5.2.11");
}
...
StmtTypeResult TypeShadowVarStmt(const ScopeContext& ctx,
                                 ...
                                 const ast::ShadowVarStmt& node,
                                 const TypeEnv& env,
                                 const ExprTypeFn& type_expr,
                                 const IdentTypeFn& type_ident,
                                 const PlaceTypeFn& type_place) {
  SpecDefsShadowVarStmt();
  ...
  if (node.type_opt) {
    ...
    // Introduce shadowing binding with 'var' mutability
    std::vector<std::pair<std::string, TypeRef>> binds;
    binds.emplace_back(node.name, ann.type);
    const auto intro = IntroAll(env, binds, ast::Mutability::Var, true);
    if (!intro.ok) {
